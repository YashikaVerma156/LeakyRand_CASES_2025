...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_for_err_corr_both_algo_2core_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized_update_arr_size_LLC_size_sensitivity.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} ${LLC_num_blocks}
...
./trigger_LR_WITH_ERR_CORR_both_algo_with_benchmark_suit_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized_LLC_size_sensitivity.sh ${unroll_fact} ${START_POINT} ${END_POINT} ${num_prl} ${benchmark_test} ${string_size} ${other_blocks} ${arr_size} ${algo_trigger_point} ${hit_miss_threshold} ${err_corr_iterations} ${LLC_num_blocks}
