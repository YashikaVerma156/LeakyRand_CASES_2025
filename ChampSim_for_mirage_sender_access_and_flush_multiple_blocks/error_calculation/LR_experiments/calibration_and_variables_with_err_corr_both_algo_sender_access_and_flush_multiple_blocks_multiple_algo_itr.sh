...
if [ ${benchmark_test} -eq 1 ]; then
  ...
  START_POINT=62   #1     ## Read START_POINT and END_POINT of the benchmark, as a reference for the seed and the benchmark to be used.
  END_POINT=62     #250 
fi
num_prl=11        ## Number of parallel instances to be run, depends on the number of cores.
	...
	if [ $unroll_fact -eq 16 ]; then
      echo " other blocks are: $other_blocks"
          ...
          other_blocks=29
					...
					arr_size=261272    #261280   #261352,when err_corr_it==1 and other_blocks=32   #261272                    #261352
          other_blocks=30
      fi
...
./trigger_LR_WITH_ERR_CORR_both_algo_with_benchmark_suit_sender_access_and_flush_multiple_blocks_multiple_algo_itr.sh ${unroll_fact} ${START_POINT} ${END_POINT} ${num_prl} ${benchmark_test} ${string_size} ${other_blocks} ${arr_size} ${algo_trigger_point} ${hit_miss_threshold} ${err_corr_iterations}
