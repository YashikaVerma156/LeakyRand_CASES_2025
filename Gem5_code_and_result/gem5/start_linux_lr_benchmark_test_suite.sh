...
mkdir -p results_gem5_lr/result_${benchmark}

parallel_run_count=0

    for (( i=301; i<=${total_count}; i++ ));
    do
        mkdir -p results_gem5_lr/result_${benchmark}/results_strnum_$i
	...
	sudo ./build/X86/gem5.opt --outdir=/data/gem5_experiments_setup_ugp/gem5/results_gem5_lr/result_${benchmark}/results_strnum_$i configs/example/gem5_library/x86-lre2e_test_suite.py --T $i &
