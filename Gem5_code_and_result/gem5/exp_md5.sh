    ...
    do
        ...
        do
            ...
            do
                ...
                directory_name="Apr21_with_flush_md5_results_X${X}_T${T}_${file}_iter${i}"

                echo "Launching experiment: X=$X, T=$T, File=$file, Iteration $i -> Directory: $directory_name"
                mkdir -p "$directory_name"
                build/X86/gem5.opt -d "$directory_name" configs/example/gem5_library/x86-ugp-fs-md5.py --X "$X" --T "$T" --file "$file" &
