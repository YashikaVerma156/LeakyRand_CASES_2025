    ...
    do
        ...
        do
            ...
            do
                ...
                directory_name="sleep16Apr_sleep${sleep_ms}ms_iter${i}_${timestamp}"

                echo "Launching experiment: X=$X, T=$T, Sleep=${sleep_ms}ms, Iteration $i -> Directory: $directory_name"
                mkdir -p "$directory_name"

                build/X86/gem5.opt -d "$directory_name" configs/example/gem5_library/x86-ugp-fs-generic.py --exec "sleep" --X "$X" --T "$T" --time "$sleep_ms" &
