...
if [ $bench_test == 0 ]
then
		search_string2="_arr_size_${sender_arr_size}_no_diff_sig_seed_train"
elif [ $bench_test == 1 ]
then
		search_string2="_arr_size_${sender_arr_size}_no_diff_sig_seed_test"
fi
sim_count=0
...
if [ $bench_test == 0 ]
then
    sender_trace=champsim.trace_sender_${string_size}_${string_num}_${sim_count}_arr_size_${sender_arr_size}_no_diff_sig_seed_train.gz
elif [ $bench_test == 1 ]
then
    sender_trace=champsim.trace_sender_${string_size}_${string_num}_${sim_count}_arr_size_${sender_arr_size}_no_diff_sig_seed_test.gz
fi
...
then
    ...
    cd ${local_dir}
    ./run_build_1core_without_diff_sig.sh ${sim_count_receiver} ${trace1} ${seed} ${string_size} ${repetition_coding} ${differential_signalling} ${llc_sets} ${path_to_copy_trace} &
...
then
    ./run_build_2core_without_diff_sig.sh ${sim_count} ${trace1} ${sender_trace} ${seed} ${string_size} 0 0 ${repetition_coding} ${differential_signalling} ${LLC_size} ${llc_sets} ${path_to_copy_trace} &
