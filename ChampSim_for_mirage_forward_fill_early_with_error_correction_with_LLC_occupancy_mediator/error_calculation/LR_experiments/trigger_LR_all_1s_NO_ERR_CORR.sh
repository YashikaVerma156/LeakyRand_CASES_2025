...
./script_for_all_1s_no_err_corr.sh ${benchmark_test} ${arr_size} ${unroll_fact}
...
do
    ...
    do
            ...
            search_string="champsim.trace_sender_512_all1s_[0-9]+.gz"
            sim_count=0
            # Use a loop to iterate through each file in the directory
            for filename in $(ls "$trace_dir"/$search_string); do
            ...
            sender_trace=champsim.trace_sender_${string_size}_all1s_${sim_count}.gz
            ./run_2core_LR_NO_ERR_CORR.sh $sim_count $receiver_trace $sender_trace $seed $string_size $STR_NUM ${benchmark_test} $trace_dir $trace_dir &
