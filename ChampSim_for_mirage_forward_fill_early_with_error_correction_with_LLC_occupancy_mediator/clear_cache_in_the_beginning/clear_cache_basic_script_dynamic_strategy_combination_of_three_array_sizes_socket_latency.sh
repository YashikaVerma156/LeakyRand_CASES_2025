...
if [ "$#" -ne 8 ]; then
    echo "Usage: $0 <mid_number1> <arr_num_itr_mid_1> <mid_number2> <arr_num_itr_mid_2> <arr_sizes> <arr_num_itr> <with_socket_latency> <with_socket_latency_sync>"
    ...
    echo "for example: <  ./clear_cache_basic_script_dynamic_strategy_combination_of_three_array_sizes_socket_latency.sh 1.1 4 1.2 1 1.3 7 1 0>  :  1.1x of LLC and 4 iterations of 1.1x"
    exit 1
fi
        ...
        if [ "$7" -eq 1 ]; then
             latency=1100
             result_file="result_random_1_champsim.trace_clear_cache_${arr_size_multiples}_dynamic_strategy_${mid_number1}_${num_iterations_mid1}_${mid_number2}_${num_iterations_mid2}_${arr_size_multiples}_${num_iterations}.gz_${latency}_${with_skt_lat}_${with_skt_lat_sync}.txt"
        fi
        if [ "$8" -eq 1 ]; then
             latency=5600
             result_file="result_random_1_champsim.trace_clear_cache_${arr_size_multiples}_dynamic_strategy_${mid_number1}_${num_iterations_mid1}_${mid_number2}_${num_iterations_mid2}_${arr_size_multiples}_${num_iterations}.gz_${latency}_${with_skt_lat}_${with_skt_lat_sync}.txt"
