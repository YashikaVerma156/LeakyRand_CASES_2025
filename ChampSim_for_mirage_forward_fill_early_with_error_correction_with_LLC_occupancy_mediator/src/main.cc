...
int Calculate_LLC_occupancy = 0, DONT_INC_ARR = 0, fence_is_seen=0, Calculate_LLC_occupancy_dyn_algo=0;
// Calculate LLC occupancy for initial set-up.
...
int cycles_to_wait_for_before_triggering_clflush_with_skt_lat = 1100;
int cycles_to_wait_for_before_triggering_clflush_with_skt_lat_sync = 5600;
...
// ================ For LLC =================
        ...
        if(flag != 1) assert(0); // sanity check
...
// =========== Calculate LLC occupancy for initial set-up.
      // =========== Calculate LLC occupancy for initial set-up.
        if(CLEAR_CACHE && Calculate_LLC_occupancy_dyn_algo == 1 && ooo_cpu[i].num_retired >= fence_instr[i] && fence_is_seen <= DYNAMIC_ALGO_ITR)
        {
            //Calculate_LLC_occupancy ++; 
            Calculate_LLC_occupancy_dyn_algo =0;
            uint64_t cnt = 0;
            cout<<"FIRST_FENCE: "<<FIRST_FENCE<<" Calculated dyn_algo LLC occupancy is : ";
            for(uint32_t i1=0; i1 < uncore.LLC.NUM_SET; i1++)
            {
              for(uint32_t j1=0; j1 < uncore.LLC.NUM_WAY; j1++)
              {
...
// =========== Calculate LLC occupancy for initial set-up.
            ...
            { 
                ...
                cout <<" received Switched on LLC occpancy check phase"<<endl;
							  cout <<" cycle: "<<current_core_cycle[i]<< " bit received is: "<< bit_revealed_is << " before llc_occ_chk" << endl; 
            ...
            { 
                // Switch-on the phase
                if(bit_revealed_is != 1) //The bit at error_correction point is always 1 to help the receiver identify whether to start error_correction phase or not.
                ...
                // Reset the flag counter.
                seen_error_correction --;//This ensure variable value oscillate betwn 2 and 1, as there is a redundant call to error_correction in the beginning to bring instructions and data blocks of this call to cache.
							  cout <<" cycle: "<<current_core_cycle[i]<< " bit received is: "<< bit_revealed_is <<" , before error_correction, hence error_correction phase is: "<<(unsigned)ERROR_CORRECTION_PHASE<< endl; 
							  //receiver_checks_for_a_miss = 0; 
							  bit_revealed_is=0; 
            }            
          ...
          {     if(NUM_CPUS > 1 && SETUP_DONE[0] == 1 && SETUP_DONE[1] == 1) //XXX Signify that receiver is done with cache prime and sender is ready to send first bit.  
               ...
               /* else
                ...
                if (ooo_cpu[i].ROB.occupancy < ooo_cpu[i].ROB.SIZE  && cpu_on_wait[i] == 0 && fence_called_on_cpu[i] == 0) //cpu_on_wait is 1 when core is on sleep
                {
                    ...
                    // handle branch
                    if(CLEAR_CACHE_WITH_SKT_LAT == 1 || CLEAR_CACHE_WITH_SKT_LAT_SYN == 1)
                    { 
                        if (ooo_cpu[i].fetch_stall == 0 && clflush_on_cpu[0][0] == 0)
