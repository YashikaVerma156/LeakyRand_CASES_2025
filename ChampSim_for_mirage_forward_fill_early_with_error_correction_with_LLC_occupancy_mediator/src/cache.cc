...
uint32_t CACHE::extractSet(uint64_t addr, uint32_t skew_id) // This is just address, don't remove extra bits ?
{
 ...
 // cout << "Encrypted line number is: " << (encrypted_line_num) << endl;
  ...
  //cout << "cacheset returned for skew id: "<<skew_id << " is: " << cacheset <<" addr: "<<addr<<" lines_in_mem: "<<lines_in_mem<<" physical_line_num: "<<physical_line_num<<endl;
  return  cacheset;
}
...
{		
    ...
    if(cache_type == IS_LLC && /*FIRST_TIME_CACHE_SETUP_DONE &&*/ match_way == UINT32_MAX && (CHANNEL_BW_ERR_PRINT || 
                                                       ((LR_ERR_CORR_BOTH_ALGO || LR_NO_ERR_CORR) && CACHE_FILL_DONE) || FA_LRU_CACHE) 
      )
    {
    ...
    if(cache_type == IS_LLC && /*FIRST_TIME_CACHE_SETUP_DONE &&*/ match_way == UINT32_MAX && IEEE_SEED) 
    {
        //Stat for SPP.
