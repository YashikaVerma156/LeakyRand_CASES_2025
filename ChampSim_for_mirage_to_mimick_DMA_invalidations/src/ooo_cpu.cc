...
void O3_CPU::handle_branch()
{
    ...
    while (continue_reading) {
        ...
        /*        else if(LLC_OCC_CHK_PHASE == 1 && ((seen_rdtsc == 2 && instr_flushed_beginning < EXTRA_ARR_SIZE) || seen_rdtsc == 4) && instr_flushed_beginning != UNROLLING_FACTOR) 
                ...
                //@IO_INVALIDATIONS
                if(current_instr.fence == 1 && seen_fence_once == 0)
                {   seen_fence_once = 1; 
                    cout << "seen_fence_once: "<<seen_fence_once<<endl;
                }
                ...
                if(current_instr.wait == 1)
                {
                    ...
                    {
                        //This check is to assure that the noise core does not implement wait.
  ...
  //Assuming HIT_MISS_THRESHOLD for UNROLLING_FACTOR == 1 and HIT_MISS_THRESHOLD_LLC_OCC_CHK are the same.
                ...
                //@IO_INVALIDATIONS
                if(current_instr.ipc_count_start == 1)
                {
                    if(counting_instr_for_ipc == 0)
                    {    
                        IO_instruction_start = instr_unique_id;
                        counting_instr_for_ipc = 1;
                        start_IO_cycle = current_core_cycle[cpu];
                    }
                    Total_IO_instr ++;
                }
                if(current_instr.ipc_count_start == 0 && counting_instr_for_ipc == 1)
                {
                    counting_instr_for_ipc = 0;
                    IO_instruction_end = instr_unique_id;
                }
                arch_instr.instr_id = instr_unique_id;
		if(DETAILED_PRINT /*|| LLC_OCC_CHK_PHASE || CHANNEL_BW_ERR_PRINT*/)
		    cout<<"fptr: "<<fptr<<" IP is: "<<hex<< current_instr.ip<<dec<<" ipc_count_start: "<<current_instr.ipc_count_start<<" fence: "<<current_instr.fence<<" rdtsc: "<<current_instr.rdtsc<< " instr_id: "<<instr_unique_id<<" cpu: "<<cpu <<" cycle[cpu]: " <<current_core_cycle[cpu] <<" branch: " <<(int)current_instr.is_branch <<" taken: "<<(int)current_instr.branch_taken;
...
void O3_CPU::retire_rob()
{
    ...
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " is retired" << endl; });
				...
				//@IO_INVALIDATIONS
        if(IO_instruction_end != 0 && counting_instr_for_ipc == 0 && ROB.entry[ROB.head].instr_id == IO_instruction_end)
        {
            IO_instruction_end = 0;
            IO_instruction_start = 0;
            Total_cycles_spent_in_IO += current_core_cycle[cpu] - start_IO_cycle;
            start_IO_cycle = 0;
            cout <<"Total_IO_miss: "<<Total_IO_miss<<" IO_miss: "<<IO_miss<<endl;
            IO_miss = 0;
