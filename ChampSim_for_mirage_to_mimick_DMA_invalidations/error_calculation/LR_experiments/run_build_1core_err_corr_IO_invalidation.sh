...
sed -i.bak 's/\<PRIORITIZE_INVALID_BLOCKS 0\>/PRIORITIZE_INVALID_BLOCKS '${invalid_block_priority}'/g' inc/champsim.h
...
sed -i.bak 's/\<PRIORITIZE_INVALID_BLOCKS '${invalid_block_priority}'\>/PRIORITIZE_INVALID_BLOCKS 0/g' inc/champsim.h
...
#	(./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed}  -traces $mix1) > error_calculation/LR_experiments/result_${repl}_${CORE}_${trace1}.txt
	(./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed}  -traces $mix1) > ${Result_dir}/result_${repl}_${CORE}_${trace1}_priority_${invalid_block_priority}.txt
