...
def process_files_in_directory(directory, benchmark_file, benchmark_type, threshold):
    total_err_cnt=0
    thresholds=[]
            ...
            with open(filepath, "r") as file:
                for line_number, line in enumerate(file, start=1):
                    if re.search(pattern1, line):
                        ...
                        msg_string = str(filename.split("_")[14])
                        sender_arr_size = str(filename.split("_")[18])
...
for directory in directories_to_process:
    if os.path.exists(directory):
      ...
      ##### Create a file for later use.
        ...
        threshold_file_path="../spp_results_analysis_scripts_train_without_ref/calculate_optimal_threshold/temp_optimal_threshold_"+str(sender_array_size)+".txt"
        ...
        pattern = f"error_count_sender_arr_size_{sender_array_size}_msg_size*"
        ...
        pattern = f"error_diff_1to0_sender_arr_size_{sender_array_size}_msg_size*"
        # Get all matching files
        files_to_delete = glob.glob(os.path.join(directory_path, pattern))
        # Delete each file
        for file_path in files_to_delete:
        ...
        pattern = f"error_diff_0to1_sender_arr_size_{sender_array_size}_msg_size*"
        # Get all matching files
        files_to_delete = glob.glob(os.path.join(directory_path, pattern))
        # Delete each file
        for file_path in files_to_delete:
        ...
        process_files_in_directory(directory, benchmark_file, bnchmrk_typ, threshold)
