...
#define NUM_INSTR_INC_BLOCK 3   //2


extern uint32_t SCHEDULING_LATENCY, EXEC_LATENCY;

extern uint64_t FIRST_WAIT_FOR_SENDER, // In set-up period, sender waits for longer time.
...
class O3_CPU {
  public:
    ...
    FILE *trace_file_fl;
    char trace_string_fl[1024];
    char gunzip_command_fl[1024];

    FILE *trace_file_fl_llc_occ;
    char trace_string_fl_llc_occ[1024];
    char gunzip_command_fl_llc_occ[1024];
    
    FILE *trace_file_fen;
    char trace_string_fen[1024];
    char gunzip_command_fen[1024];

    FILE *trace_file_inc;
    char trace_string_inc[1024];
    char gunzip_command_inc[1024];

    FILE *trace_file_fen_rd;
    char trace_string_fen_rd[1024];
    char gunzip_command_fen_rd[1024];

    FILE *trace_file_arr_access;
    char trace_string_arr_access[1024];
    char gunzip_command_arr_access[1024];

    int INC_ARR_LEN, read_increased_arr_instr, FILL_HOLE;

    // instruction
    input_instr current_instr, dummy_current_instr;
    cloudsuite_instr current_cloudsuite_instr;
    uint64_t REDUCED_ARR_INC_ADDR, REDUCED_ARR_SIZE, flush_more_address, traversed_element, incremented_elements_traversed, traversed_incremented_element, LAST_ARR_INC_ADDR, FIRST_ARR_INC_ADDR;
    int ins_read_from_arr, ins_read_from_fl, ins_read_from_fen, ins_read_from_fen_rd, instr_to_read_this_time, instr_flushed_beginning, seen_fence, seen_rdtsc, access_to_fill_empty_slots, turn_to_traverse_arr, still_have_array_elements_to_traverse;
    ...
    O3_CPU() {
        ...
        trace_file_fl = NULL;
        trace_file_fen = NULL;
        trace_file_fen_rd = NULL;
        trace_file_inc = NULL;
        trace_file_arr_access = NULL;

        // instruction
        INC_ARR_LEN = 0;
        FILL_HOLE = 0;
        REDUCED_ARR_INC_ADDR = 0;
        REDUCED_ARR_SIZE = 0;
        LAST_ARR_INC_ADDR = 0; 
        FIRST_ARR_INC_ADDR = 0;
        incremented_elements_traversed = 0; 
        traversed_incremented_element = 0;
    ...
    // functions
    void handle_branch(), update_trace_file(), memorize_num_instrs_read(), fseek_manual(),
         fetch_instruction(),
