...
# Check if exactly four arguments are passed
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <other_blocks> <LLC_num_blocks> <num_prl> <str_num>"
    ...
    echo "str_num specify how many experiments per configuration will be run."
    exit 1
fi
...
if [ "$LLC_num_blocks" -eq 16384 ]; then
  if [ "$other_blocks" -eq 16 ]; then
    ...
    do
        for unroll_fact in 128 #128 64  #16   #16 32
        do
            ...
            do
                echo "====================RUNNING FOR unroll_fact: ${unroll_fact}, algo_trigger_point: ${algo_trigger_point}, benchmark_test: ${benchmark_test} other_blocks: ${other_blocks} ======================="   
                 exit 1
                ./calibration_fig12.sh ${unroll_fact} ${algo_trigger_point} ${benchmark_test} ${other_blocks} ${err_corr_iterations} ${LLC_num_blocks} ${str_num} ${num_prl}
            done
        done
    done
