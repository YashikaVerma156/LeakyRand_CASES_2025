...
def verify_error(file_path, bit_position, equivalent_num_uf_chunks_to_avoid):
    #print(file_path)
    #print(bit_position)
    bit_position=int(bit_position)+2
    # Construct the command
    ...
    for i in range(1,NUM_BITS_WITH_MARKER_BITS+1):
        if(int(original_msg[i-1]) != int(received_msg[i-1])):
            ...
            if(int(is_it_an_error) == 0):
                if(int(received_msg[i-1]) == 1):
                    received_msg[i-1] = 0
                else:
                    print("Something is wrong.","original_msg[i-1]:",int(original_msg[i-1]),"received_msg[i-1]:",int(received_msg[i-1]))
