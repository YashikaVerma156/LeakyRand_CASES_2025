...
percentage_receiver_arr=$6 
percentage_data_bits=$7
dont_probe_complete=$8
str_num=$9
num_prl=${10}
...
mkdir -p binary
mkdir -p pin_traces
mkdir -p LR_ERR_CORR_2core_results_both_algo_${err_corr_iterations}_access_${percentage_receiver_arr}_${percentage_data_bits}_${dont_probe_complete}
...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_for_err_corr_both_algo_2core_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized_update_arr_size.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} ${percentage_receiver_arr} ${percentage_data_bits} ${dont_probe_complete}
