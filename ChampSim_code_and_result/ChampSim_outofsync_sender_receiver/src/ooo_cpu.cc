                ...
                for (int i=0; i<NUM_INSTR_SOURCES; i++) 
                {
                    ...
                    if (arch_instr.source_memory[i])
				      ...
				      {  
				          ...
				          num_mem_ops++;
                          if(NUM_CPUS == 2 && dont_probe_complete == 1 && DONT_PROBE_COMPLETE == 1 && cpu == 0)
                          {    if( arch_instr.source_memory[i] <= probe_stopping_point && arch_instr.source_memory[i] >= ARR_START_ADDR)
                                  skip = 1;
                          }
