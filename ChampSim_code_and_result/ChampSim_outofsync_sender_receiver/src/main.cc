...
int CURR_ERR_CORR_ITR = 0, MISSES_OBSERVED = 0, ERROR_CORRECTION_PHASE_LAST = 0, BIT_REVEALED_LAST = 0, CURR_LLC_OCC_CHK_ITR =0, ITR_TO_FILL_HOLE=0, received_bit_number = 0, rec_arr_addr = 0, percnt_data_bit = 0, dont_probe_complete = 0;
...
uint64_t sender_evicted_own_active_block = 0, sender_evicted_own_inactive_block = 0, sender_evicted_receiver_inactive_block = 0, sender_evicted_receiver_unrecognizable_active_block = 0, sender_evicted_receiver_recognizable_active_block = 0, sender_filled_up_empty_block = 0, probe_stopping_point = 0;
...
int main(int argc, char** argv)
{
   ...
   cout <<"ALGO_TRIGGER_POINT: "<<ALGO_TRIGGER_POINT<< endl;
   if(DONT_PROBE_COMPLETE == 1)
   {
			 cout <<"PERCNT_REC_ARR: "<<PERCNT_REC_ARR<< endl;
			 cout <<"PERCNT_DATA_BIT: "<<PERCNT_DATA_BIT<< endl;
			 if(PERCNT_REC_ARR == 97)
			 {
           probe_stopping_point = 5055936; //4d25c0
       }
			 if(PERCNT_REC_ARR == 95)
			 {
           probe_stopping_point = 5097408; //4dc7c0
       }
			 if(PERCNT_REC_ARR == 92)
			 {
           probe_stopping_point = 5159616; //4ebac0
       }
			 if(PERCNT_REC_ARR == 90)
			 {
           probe_stopping_point = 5201088; //4f5cc0
       }
			 if(PERCNT_REC_ARR == 88)
			 {
           probe_stopping_point = 5242560; //4ffec0
       }
			 if(PERCNT_REC_ARR == 86)
			 {
           probe_stopping_point = 5284032; //50a0c0
       }
			 if(PERCNT_REC_ARR == 85)
			 {
           probe_stopping_point = 5304832; //50f200
       }
			 if(PERCNT_REC_ARR == 80)
			 {
           probe_stopping_point = 5408512; //528700
       }
			 percnt_data_bit = 512/((512*PERCNT_DATA_BIT)/100);
			 cout <<"percnt_data_bit: "<<percnt_data_bit<< endl;
   }
                          ...
                          {  
                              if(i == 1)
                              {
                                   ...
                                   {   
                                       ...
                                       if(DONT_PROBE_COMPLETE == 1)
                                       {
																					 dont_probe_complete = 0;
																					 received_bit_number ++; 
																					 if(received_bit_number == (ALGO_TRIGGER_POINT - 1))
																							 received_bit_number = 0;
																					 cout <<"received_bit_number: " << received_bit_number << endl;
																					 if(received_bit_number != 0 && (received_bit_number % percnt_data_bit == 0))
																					 {
																							 cout << "DONT_PROBE_COMPLETE"<<endl;
																							 dont_probe_complete = 1;
																					 }
                                       }
                                   }
                                   receiver_checks_for_a_miss = 0; 
