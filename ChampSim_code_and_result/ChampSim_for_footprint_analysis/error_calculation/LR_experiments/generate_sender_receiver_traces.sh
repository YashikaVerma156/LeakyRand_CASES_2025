...
build_count=0
		...
		cd ${pin_files_path}
		#./commands_encyption_code.sh ${sen_dis} ${path_to_copy_trace} #Reusing the already generated traces.
		sim_count_sender_aes=`grep -oE 'instrCount: [0-9]+' ${pin_output_file_aes} | awk '{print $2}'`
		echo "sim_count: $sim_count_sender_aes"
		sim_count_sender_rsa=`grep -oE 'instrCount: [0-9]+' ${pin_output_file_rsa} | awk '{print $2}'`
		echo "sim_count: $sim_count_sender_rsa"

		#trace_sender_aes=champsim.trace_aes_encrypt_decrypt.gz
		#trace_sender_rsa=champsim.trace_rsa_encrypt_decrypt.gz
		trace_sender_aes_with_wait=champsim.trace_aes_encrypt_decrypt_with_wait.gz
		trace_sender_rsa_with_wait=champsim.trace_rsa_encrypt_decrypt_with_wait.gz

		cd ${back_to_exp_dir}

    #./run_build_1core_err_corr.sh ${sim_count_sender_aes} ${trace_sender_aes_with_wait} 0 ${seed} 0 0 0 0 0 ${path_to_copy_trace} ${Result_dir}
    #./run_build_1core_err_corr.sh ${sim_count_sender_rsa} ${trace_sender_rsa_with_wait} 0 ${seed} 0 0 0 0 0 ${path_to_copy_trace} ${Result_dir}
