...
do
  ...
  s_trace=$(basename ${sender_trace})
	...
	./${binary_path}/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed} -traces ${receiver_trace} ${sender_trace} > ${result_dir}/result_${repl}_${CORE}_${r_trace}_${s_trace}_${seed}.txt
