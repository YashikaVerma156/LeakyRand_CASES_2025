...
do_err_corr_at=$4  
other_blocks=$5
err_corr_iterations=$6  #XXX Defines multiple iterations of error correction. If we assume sender create multiple holes.
...
Total=${LLC_occupancy_of_receiver}
...
do
   ...
   echo "1. ${sim_count_receiver}, 2. ${trace_receiver}, 3. ${string_size}, 4. ${seed}, 5. ${extra}, 6. ${extra_for_llc_occ_chk}, 7. ${start_arr_addr}, 8. ${max_arr_len}, 9. ${extra_arr_max_addr}, 10. ${path_to_copy_trace}, 11. ${Result_dir}"
   #exit
...
./update_arr_size.sh ${unroll_fact} ${do_err_corr_at} ${err_corr_iterations} ${other_blocks} ${receiver_arr_size}

echo "Receiver LLC occupancy: ${LLC_occupancy_of_receiver} extra_for_llc_occ_chk: ${extra_for_llc_occ_chk}" # Adjust Receiver occupancy, don't change Sender occupancy.
