...
{		
    ...
    if(cache_type == IS_LLC && /*FIRST_TIME_CACHE_SETUP_DONE &&*/ match_way == UINT32_MAX && (CHANNEL_BW_ERR_PRINT || ((LR_ERR_CORR_BOTH_ALGO || LR_NO_ERR_CORR) && CACHE_FILL_DONE) || CACHE_REGION_IDENTIFICATION || NEW_COVERT_CHANNEL))
    {
        //Stat for SPP.
        LLC_MISS_COUNT[packet->cpu] ++;

        if(packet->cpu == 0 && FIRST_TIME_CACHE_SETUP_DONE) // stat collected only for receiver.
        {    misses_in_a_chunk ++;
             //cout <<"miss CPU: "<<packet->cpu <<" instr_id: "<<packet->instr_id <<endl; 
        }
