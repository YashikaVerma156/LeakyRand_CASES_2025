...
void O3_CPU::handle_branch()
{
    ...
    while (continue_reading) {
        ...
        /*        else if(LLC_OCC_CHK_PHASE == 1 && ((seen_rdtsc == 2 && instr_flushed_beginning < EXTRA_ARR_SIZE) || seen_rdtsc == 4) && instr_flushed_beginning != UNROLLING_FACTOR) 
                ...
                if(current_instr.fence == 1)
                {
                    if(cpu == 0) {
                        ...
                        {    
                            ...
                            cout << "FIRST_FENCE is: "<< FIRST_FENCE <<endl;
                            cout << "first fence current core cycle: "<<current_core_cycle[cpu]<<endl;
                        }
                        else
                ...
                {
                    if(CACHE_REGION_IDENTIFICATION == 1 && seen_first_rdtsc == 0)
                    {
                        seen_first_rdtsc = 1;
                    }
                    if(LLC_OCC_CHK_PHASE == 1 ) 
  ...
  //Assuming HIT_MISS_THRESHOLD for UNROLLING_FACTOR == 1 and HIT_MISS_THRESHOLD_LLC_OCC_CHK are the same.
                       ...
                       // Checking for multiple misses during LLC_OCC_CHK_PHASE  //XXX can be modified later to identify >1 misses and inc flush_more_address by that.
                            ...
                            #endif
                            cout<<"RDTSC, sub: "<<(current_core_cycle[cpu] - prev_rdtsc_counter)<<" prev: "<<prev_rdtsc_counter<<" cycle: "<<current_core_cycle[cpu]<<" id: "<<prev_rdtsc_id<<" - "<<instr_unique_id<<" misses: "<<misses_in_a_chunk <</*" repetetion_coding: "<<REPETETION_CODING<<" differential_signalling: "<<DIFFERENTIAL_SIGNALLING<<*/endl;
                    ...
                    { 
                        #if(CLEAR_CACHE || CACHE_REGION_IDENTIFICATION)
