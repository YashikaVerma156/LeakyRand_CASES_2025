...
int CURR_ERR_CORR_ITR = 0, MISSES_OBSERVED = 0, ERROR_CORRECTION_PHASE_LAST = 0, BIT_REVEALED_LAST = 0, CURR_LLC_OCC_CHK_ITR =0, ITR_TO_FILL_HOLE=0, NUM_CHUNKS_WITH_MISSES = 0;
...
uint64_t sender_evicted_own_active_block = 0, sender_evicted_own_inactive_block = 0, sender_evicted_receiver_inactive_block = 0, sender_evicted_receiver_unrecognizable_active_block = 0, sender_evicted_receiver_recognizable_active_block = 0, sender_filled_up_empty_block = 0, misses_in_a_chunk = 0;
...
uint64_t seen_first_rdtsc = 0;

uint32_t count_array_blocks_in_LLC()
{
    //cout<<"new array_start_address: "<<ARR_START_ADDR<<" arr_size_probed: "<<ARR_SIZE_PROBED << endl;
    uint32_t hits_in_llc=0;
    uint64_t start_address = ARR_START_ADDR, pa;
    for(int i = 0; i < int(ARR_SIZE_PROBED*uncore.data_LLC.NUM_WAY); i++)
    {
        pa = va_to_pa(0, 0, start_address, start_address >> LOG2_PAGE_SIZE);       
        start_address += 64;
        for(uint32_t i1=0; i1 < uncore.data_LLC.NUM_SET; i1++)
        {
            for(uint32_t j1=0; j1 < uncore.data_LLC.NUM_WAY; j1++)
            {
                if((uncore.data_LLC.block[i1][j1].tag == pa >> 6) && /*(uncore.data_LLC.block[i1][j1].full_addr == pa) &&*/ (uncore.data_LLC.block[i1][j1].valid)) 
                    hits_in_llc++;
            }
        }
...
void calculate_llc_occupancy_for_specific_cpu(uint64_t FIRST_FENCE_CPU_0, uint32_t cpu_completed_execution )
{
		uint64_t cnt1 = 0, cnt = 0;
	cout<<"Completed exec cpu: "<<cpu_completed_execution<<" Calculated LLC occupancy for cpu 0 and cpu 1 is: ";
        ...
        if((Calculate_LLC_occupancy == 1 /*|| Calculate_LLC_occupancy == 3*/) && ooo_cpu[i].num_retired >= fence_instr[i])
        {
            ...
            //cout<<"FIRST_FENCE: "<<FIRST_FENCE<<" Calculated LLC occupancy is : ";
						for(uint32_t i1=0; i1 < uncore.LLC.NUM_SET; i1++)
						{
							for(uint32_t j1=0; j1 < uncore.LLC.NUM_WAY; j1++)
							{
									if(uncore.LLC.block[i1][j1].valid == 1 && uncore.LLC.block[i1][j1].instr_id > FIRST_FENCE && uncore.LLC.block[i1][j1].cpu == 0)
								  {
												cnt ++;
									}
							}
						}
            cout <<"cnt: "<<cnt<<" LLC percentage occupied: "<<((float)cnt/uncore.data_LLC.NUM_WAY)*100 <<" current_core_cycle: "<<current_core_cycle[i]<<endl;
        }
...
// =========== Calculate LLC occupancy for initial set-up.
                ...
                { 
                          ...
                          {  
                              if(i == 1)
                              {
                                   ...
                                   {   
                                       if(NEW_COVERT_CHANNEL_LR_PROBE == 1) 
                                       {
                                           cout <<"NUM_CHUNKS_WITH_MISSES: "<<NUM_CHUNKS_WITH_MISSES<< endl;
                                           NUM_CHUNKS_WITH_MISSES = 0;
                                       }
                          ...
                          #if(IEEE_SEED || NEW_COVERT_CHANNEL)
                          ...
                          if(FIRST_FENCE != 0)
                          {
                              if(i == 0) // CPU awakened from sleep is 0, count occupancy of CPU 1.
                                  calculate_llc_occupancy_for_specific_cpu(FIRST_FENCE, 1);
                              else if(i == 1)// CPU awakened from sleep is 1, count occupancy of CPU 0.
                                  calculate_llc_occupancy_for_specific_cpu(FIRST_FENCE, 0);
                          }
                          #endif
                }
		...
		#if(CACHE_REGION_IDENTIFICATION)
		    if(seen_first_rdtsc == 1)
				{  
				    cout << "counted hits in LLC: "<< count_array_blocks_in_LLC()<<endl;
						seen_first_rdtsc += 1;
				}
		#endif
    }
...
cout << "hit_blocks: "<<hit_block<<endl;
