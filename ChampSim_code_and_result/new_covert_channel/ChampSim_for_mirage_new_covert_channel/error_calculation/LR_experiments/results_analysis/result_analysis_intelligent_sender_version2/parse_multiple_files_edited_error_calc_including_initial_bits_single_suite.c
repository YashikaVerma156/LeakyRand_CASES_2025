...
void write_absolute_and_delta_latency(int bit_num, int subspace, int histlen, int delta_lat, int abs_lat, int sender_ds, int benchmrk_typ, int sender_type)
{
    char directory[100];   
    char sender[10];
    if(sender_type == 0)
    {    
        ...
        sprintf(directory, "dist_plot_data_histlen_%d_sender_ds_%d_%s", histlen, sender_ds, sender);
    }
    ...
    char latency_filename[200];

    if (benchmrk_typ == 0)
    {
        sprintf(latency_filename, "%s/DS_%d_histlen_%d_subspace_%d_single_train_%s.txt", directory, sender_ds, histlen, subspace, sender);
    }
    else if (benchmrk_typ == 1)
    {
        sprintf(latency_filename, "%s/DS_%d_histlen_%d_subspace_%d_single_test_%s.txt", directory, sender_ds, histlen, subspace, sender);
    }
    else
    {
		...
		FILE *fptr = fopen(latency_filename, "a");
		if (fptr == NULL) {
				printf("Error opening file!\n");
				_exit(1);
		} 
    fprintf(fptr,"%d %d %d\n",bit_num, delta_lat, abs_lat);
    fclose(fptr);
      // bit number, absolute latency, and delta latency to be written in file.
}
...
int main(int argc, char *argv[])
{
  FILE *fptr;
	if (argc != 8) {
		printf("Usage: ./parse <benchmark file train/test> <dir_for_misses_file_train/test_suite> <history_length> <unrolling_factor> <threshold(n)> <less_miss: 0(more_miss), 1(less_miss)> <sender_type: 0(naive), 1(version1), or 2(version2)> .\nAborting...\n");
		exit(0);
	}
  // Open the directory
  ...
  char filepath[MAX_PATH];
	int i, j, k, benchmark_test, sender_type = atoi(argv[7]), uf = atoi(argv[4]), threshold_n = atoi(argv[5]), less_miss = atoi(argv[6]) ;
	int histlen = atoi(argv[3]), sender_ds = 0;
	assert(histlen >= 0);
	assert(sender_type >= 0 && sender_type <= 2);
  char out_filename[200], sender[20];
	//exit(0);
  ...
  if (strstr(argv[1], "benchmark_train.txt") == NULL && (strstr(argv[1], "benchmark_test.txt")) == NULL ) 
  {
      printf("Correct benchmark file not passed.\n");
      exit(1);
  }
  //exit(0);
  // Check if "train" is present
  if (strstr(argv[1], "train") && strstr(argv[2], "train")) 
  {
      if (sscanf(argv[2], "misses_results_%d_train", &sender_ds) == 1)
          printf("\"train\" is present in the argument.\n");
			sprintf(out_filename, "DS_%d_histlen_%d_%d_%d_single_train_suite_%d_%s.txt", sender_ds, histlen, uf, threshold_n, less_miss, sender);
			printf("%s\n",out_filename);
      benchmark_test = 0;
  } 
  else if (strstr(argv[1], "test") && strstr(argv[2], "test")) 
  {
      if (sscanf(argv[2], "misses_results_%d_test", &sender_ds) == 1)
          printf("\"test\" is present in the argument.\n");
			sprintf(out_filename, "DS_%d_histlen_%d_%d_%d_single_test_suite_%d_%s.txt", sender_ds, histlen, uf, threshold_n, less_miss, sender);
			printf("%s\n",out_filename);
      benchmark_test = 1;
  }
  else
	...
	while(fgets(buffer, sizeof(buffer), fp) != NULL) 
  {
      /*if (fgets(buffer, sizeof(buffer), fp) == NULL) 
      {
          printf("in if %d\n",count2);
          printf("%s\n",buffer);
          perror("Error reading header 314\n");
          fclose(fp);
          return 1;
      }*/
	...
	for (i=((histlen == 0) ? 1 : histlen); i<count; i++) 
  {
      ...
      if ((latency[i] - latency[i-1]) < minAB[j]) 
          minAB[j] = latency[i] - latency[i-1];

      if ((latency[i] - latency[i-1]) > maxAB[j]) 
          maxAB[j] = latency[i] - latency[i-1];

      printf("i: %d %d\n", i, bits[i]);
      
      write_absolute_and_delta_latency(i, j, histlen, (latency[i] - latency[i-1]), latency[i], sender_ds, benchmark_test, sender_type);
      // The name of file to be opened should be decided from the j, DS and histlen.
      // bit number, absolute latency, and delta latency to be written in file.
    ...
    {   
        ...
        fprintf(fptr,"%d %d", global_minAB[j], global_maxAB[j]);
        if(j%2 == 0)
            fprintf(fptr," ");
    }
    printf("\n");
 ...
 //=======The code below is for calculating best threshold and errors.\n");
  ...
  printf(" ======== Minimum errors and best threshold observed. ======== \n");
  fprintf(fptr," ======== Minimum errors and best threshold observed. ======== \n");
	for (j=0; j<(1 << histlen); j++) 
  {
    ...
    else if(global_maxAB[2*j] >= global_minAB[2*j+1] && global_minAB[2*j+1] > global_minAB[2*j] && global_maxAB[2*j+1] > global_maxAB[2*j])
    {
    ...
    else
    {
        ...
        printf("A different scenario is observed, min[2j]: %d, max[2j]: %d, min[2j+1]: %d, max[2j+1]: %d\n", global_minAB[2*j], global_maxAB[2*j], global_minAB[2*j+1], global_maxAB[2*j+1]);
        assert(0);
    }
    // XXX New condition.
