        ...
        with open(file_path, "r") as file:
            ...
            for line_number, line in enumerate(file, start=1):
                if re.search(pattern, line):
                    # Skip the first occurrence of the string "Total Error: ".
...
# Counting the range of optimal threshold.
    th_range=[]
    for fname in file_list:
        file_path = os.path.join(src_dir, fname)
        pattern="Total Error: "
...
#3. Print them.
    ...
    if(size == 9830):
        print(th_range)
    print("size: ", size, ' min_err_count: ', min_err_count, ' Threshold: ', Threshold, ' range length: ',len(th_range))
