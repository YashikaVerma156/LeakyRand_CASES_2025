...
int main(int argc, char *argv[])
{
  FILE *fptr;
	if (argc != 7) {
		printf("Usage: ./parse <benchmark file train> <dir_for_latency_file_train_suite> <history_length> <dir_for_latency_file_test_suite> <benchmark file test> <sender_type: 0(naive), 1(version1), or 2(version2)>.\nAborting...\n");
		exit(0);
	}
  // Open the directory
  ...
  char filepath[MAX_PATH];
	int i, j, k;
	int histlen = atoi(argv[3]), sender_type = atoi(argv[6]), sender_ds = 0;
	assert(histlen >= 0);
  ...
  if (sscanf(argv[2], "data_%d_train_suite", &sender_ds) == 1) {
  ...
  sprintf(out_filename, "DS_%d_histlen_%d_%s.txt", sender_ds, histlen, sender);
  printf("%s\n",out_filename);
