...
if len(sys.argv) != 4:
    ...
    print("Second argument is for noise trigger point.")
    print("Third argument is err_corr_iterations.")
    sys.exit(1)
...
noise_trigger_point=int(sys.argv[2])
...
directory_name = 'extracted_results_'+str(noise_trigger_point)+'_'+str(err_corr_iterations)
...
string_number=0
per_string_data={}
directory_path='LR_ERR_CORR_2core_results_both_algo_'+str(err_corr_iterations)+'_access'

res_dir_path='extracted_results_'+str(noise_trigger_point)+'_'+str(err_corr_iterations)
...
for UF in UFs:
    for atp in Algo_trigger_point:
        ...
        string2='_'+str(atp)+'_with_both_algo_'+str(err_corr_iterations)+'_access_train.gz_noise_point_'+str(noise_trigger_point)+'.txt'
        string3='_'+str(atp)+'_with_both_algo_'+str(err_corr_iterations)+'_access_test.gz_noise_point_'+str(noise_trigger_point)+'.txt'
        if(UF == 32):
            TH = 134
        elif(UF == 16):
            TH = 106
	...
	#Declaring empty dictionary to store data per message strings that are communicated.
        per_string_data={}
        if train_data == 1:
            tot_err_cnt_file='total_error_count_UF_'+str(UF)+'_'+str(atp)+'_train_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_noise_point_'+str(noise_trigger_point)+'.csv'
            ...
            tot_err_cnt_file='total_error_count_UF_'+str(UF)+'_'+str(atp)+'_test_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_noise_point_'+str(noise_trigger_point)+'.csv'
            # Filter files that contain the specified string
        ...
        for file_name in filtered_files:
            ...
            if train_data == 1:
                data_file='per_string_data_strnum_'+str(string_number)+'_UF_'+str(UF)+'_'+str(atp)+'_train_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_noise_point_'+str(noise_trigger_point)+'.csv'
            else:
                data_file='per_string_data_strnum_'+str(string_number)+'_UF_'+str(UF)+'_'+str(atp)+'_test_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_noise_point_'+str(noise_trigger_point)+'.csv'
            ...
            for line_number, line in enumerate(file, start=1):
                ...
                if(line_number == 2):
                    ...
                    if(int(line[11]) < int(min_algo2)):
                        ...
                        #exit(1)
            text_file='Total_result_'+str(UF)+'_train_'+str(train_data)+'_'+str(atp)+'_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_noise_point_'+str(noise_trigger_point)+'.txt'
