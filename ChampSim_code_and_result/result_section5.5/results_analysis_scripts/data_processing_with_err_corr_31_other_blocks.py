...
if len(sys.argv) != 4:
    print("Three commandline arguments are expected i.e")
    print("1) 1 if the training_dataset results are processed or 0 if the testing_dataset results are processed.")
    print("2) 1 if only Sender pays socket latency, 0 otherwise.")
    print("3) 1 if Everyone pays socket latency, 0 otherwise.")
    print("Arguments 2nd and 3rd can't be 1 simultaneously.        Exiting...")
    sys.exit(1)
...
if int(sys.argv[2]) > 1 or int(sys.argv[2]) < 0:
    print("The value should be either 0 or 1.")
    sys.exit(1)
...
if int(sys.argv[2]) == 1 and int(sys.argv[3]) == 1:
    print("Both the values can't be 1 together.")
    sys.exit(1)

# Access command-line argument.
train_data=int(sys.argv[1])
sen_skt_lat=int(sys.argv[2])
evr_skt_lat=int(sys.argv[3])

#UFs=[16]
UF=16
socket_latencies=[5600]
#Algo_trigger_point=[512, 256, 128, 64, 32, 16, 8]
#Algo_trigger_point=[16,32,64,128,256,512]
#Algo_trigger_point=[514]
Algo_trigger_point=[128, 512]
string_len=512
...
for skt_lat in socket_latencies:
    for atp in Algo_trigger_point:
        ...
        if atp == 512:
            other_blocks=31
            directory_path='../LR_ERR_CORR_2core_results_both_algo_9_access'
        file_list = os.listdir(directory_path)
        if atp == 8 or atp == 16 or atp == 32:
            ...
            err_corr_iterations=7  #Maximum number of times error-correction can be repeated.
            
        print('processing for UF: ',UF,' Algo_trigger_point: ',atp)
        string='multiple_access_'+str(UF)+'_other_blocks_'+str(other_blocks)+'_'
        string2='_'+str(atp)+'_with_both_algo_'+str(err_corr_iterations)+'_access_train.gz_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.txt'
        string3='_'+str(atp)+'_with_both_algo_'+str(err_corr_iterations)+'_access_test.gz_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.txt'
        if(UF == 32):
            TH = 134
        elif(UF == 16):
            TH = 106
	...
	#Declaring empty dictionary to store data per message strings that are communicated.
        per_string_data={}
        if train_data == 1:
            tot_err_cnt_file='total_error_count_UF_'+str(UF)+'_'+str(atp)+'_train_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.csv'
            ...
            tot_err_cnt_file='total_error_count_UF_'+str(UF)+'_'+str(atp)+'_test_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.csv'
            # Filter files that contain the specified string
        ...
        for file_name in filtered_files:
            ...
            if train_data == 1:
                data_file='per_string_data_strnum_'+str(string_number)+'_UF_'+str(UF)+'_'+str(atp)+'_train_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.csv'
            else:
                data_file='per_string_data_strnum_'+str(string_number)+'_UF_'+str(UF)+'_'+str(atp)+'_test_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.csv'
            ...
            for line_number, line in enumerate(file, start=1):
                ...
                if(line_number == 2):
                    ...
                    if(int(line[11]) < int(min_algo2)):
                        ...
                        #exit(1)
            text_file='Total_result_'+str(UF)+'_train_'+str(train_data)+'_'+str(atp)+'_with_err_corr_both_algo_'+str(err_corr_iterations)+'_access_other_blocks_'+str(other_blocks)+'_'+str(skt_lat)+'_'+str(sen_skt_lat)+'_'+str(evr_skt_lat)+'.txt'
