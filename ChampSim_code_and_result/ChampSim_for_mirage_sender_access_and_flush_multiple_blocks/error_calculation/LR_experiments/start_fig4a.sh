...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <other_blocks> <str_num> <num_parallel_simulations>"
    echo "other_blocks should be either 31 or 326"
    echo "str_num defines the number of experiments to be run per configuration."
		...
		do
			...
			do
				...
				echo "====================RUNNING FOR unroll_fact: ${unroll_fact}, algo_trigger_point: ${algo_trigger_point}, benchmark_test: ${benchmark_test} other_blocks: ${other_blocks} ======================="   
                                #exit 1 
                                ./calibration_fig4a.sh ${unroll_fact} ${algo_trigger_point} ${benchmark_test} ${other_blocks} ${err_corr_iterations} ${num_prl} ${str_num}
		        done
	        done
        done
