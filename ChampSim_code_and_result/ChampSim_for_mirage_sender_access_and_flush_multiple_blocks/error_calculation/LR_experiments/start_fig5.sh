...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <other_blocks> <num_parallel_simulations> <str_num>"
    ...
    echo "str_num control the number of experiments per configuration."
    exit 1
fi
				...
				do
						...
						do
		                                     ...
		                                     echo "====================RUNNING FOR unroll_fact: ${unroll_fact}, algo_trigger_point: ${algo_trigger_point}, benchmark_test: ${benchmark_test} other_blocks: ${other_blocks} ======================="   
                                                     #exit 1 
		                                     ./calibration_fig5.sh ${unroll_fact} ${algo_trigger_point} ${benchmark_test} ${other_blocks} ${err_corr_iterations} ${num_prl} ${str_num}
						done
				done
		done
done
