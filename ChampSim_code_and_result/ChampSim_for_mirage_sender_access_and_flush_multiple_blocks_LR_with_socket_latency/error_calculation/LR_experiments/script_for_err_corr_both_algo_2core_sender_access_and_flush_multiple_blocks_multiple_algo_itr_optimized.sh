...
# Check if exactly 11 arguments are passed
if [ "$#" -ne 11 ]; then
...
sed -i.bak 's/\<SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD 0\>/SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD '${sync_overhead}'/g' ../../inc/champsim.h
sed -i.bak 's/\<SENDER_PAYS_SOCKET_LATENCY 0\>/SENDER_PAYS_SOCKET_LATENCY '${snd_skt_lat}'/g' ../../inc/champsim.h
sed -i.bak 's/\<EVERYONE_PAYS_SOCKET_LATENCY 0\>/EVERYONE_PAYS_SOCKET_LATENCY '${evr_skt_lat}'/g' ../../inc/champsim.h
...
path_to_copy_trace=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_LR_with_socket_latency/error_calculation/LR_experiments/pin_traces
back_to_exp_dir=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_LR_with_socket_latency/error_calculation/LR_experiments
Result_dir=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_LR_with_socket_latency/error_calculation/LR_experiments/results
...
then
    ...
    sed -i.bak 's/\<SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD '${sync_overhead}'\>/SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD 0/g' ../../inc/champsim.h
    sed -i.bak 's/\<SENDER_PAYS_SOCKET_LATENCY '${snd_skt_lat}'\>/SENDER_PAYS_SOCKET_LATENCY 0/g' ../../inc/champsim.h
    sed -i.bak 's/\<EVERYONE_PAYS_SOCKET_LATENCY '${evr_skt_lat}'\>/EVERYONE_PAYS_SOCKET_LATENCY 0/g' ../../inc/champsim.h
...
do
  ...
  result_file_path=results/${result_file}
  start_line=`grep -nri "is on wait" ${result_file_path} | head -n 1 | tail -n 1 | awk -F ":" '{print $1}'` 
  end_line=`grep -nri "is on wait" ${result_file_path} | head -n 2 | tail -n 1 | awk -F ":" '{print $1}'` 
...
./run_build_2core_err_corr.sh ${sim_count_sender} ${trace_receiver} ${trace_sender} ${seed} ${string_size} ${extra} ${extra_for_llc_occ_chk} ${start_arr_addr} ${max_arr_len} ${extra_arr_max_addr} ${path_to_copy_trace} ${Result_dir} ${sync_overhead} ${snd_skt_lat} ${evr_skt_lat} &
