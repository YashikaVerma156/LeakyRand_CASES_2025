...
back_to_exp_dir=../../../../../ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_LR_with_socket_latency/error_calculation/LR_experiments
...
cd ${pin_files_path} 
#./commands_sender_LR_ERR_CORR_both_algo_with_benchmark_suit_multiple_access.sh ${string_num} ${string_size} ${do_err_corr_at} ${do_llc_occ_chk_at} ${path_to_copy_trace} ${benchmark_test} ${err_corr_iterations}
...
do
  ...
  if [[ ${tim} -eq 5 ]] #3  ] #### Terminate the loop if it is running for more than 5 times.
  then
      sed -i.bak 's/\<MAX_ARR_INC '${max_inc_in_array}'\>/MAX_ARR_INC 0/g' ../../inc/config.h
      sed -i.bak 's/\<NUM_SENDER_DISTURBANCES '${err_corr_iterations}'\>/NUM_SENDER_DISTURBANCES 0/g' ../../inc/config.h
      sed -i.bak 's/\<HIT_MISS_THRESHOLD '${hit_miss_threshold}'\>/HIT_MISS_THRESHOLD 0/g' ../../inc/champsim.h
