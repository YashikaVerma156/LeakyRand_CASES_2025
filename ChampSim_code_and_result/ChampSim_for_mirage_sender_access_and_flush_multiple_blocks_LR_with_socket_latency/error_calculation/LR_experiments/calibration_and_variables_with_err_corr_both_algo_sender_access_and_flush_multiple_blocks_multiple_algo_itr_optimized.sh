...
evr_skt_lat=$8
str_num=$9
num_prl=${10}        ## Number of parallel instances to be run, depends on the number of cores.
...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_for_err_corr_both_algo_2core_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized_update_arr_size.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} ${sync_overhead} ${snd_skt_lat} ${evr_skt_lat}
