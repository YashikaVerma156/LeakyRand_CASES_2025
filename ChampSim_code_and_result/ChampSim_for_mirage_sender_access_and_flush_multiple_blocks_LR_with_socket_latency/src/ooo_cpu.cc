    ...
    while (continue_reading) {
        ...
        /*        else if(LLC_OCC_CHK_PHASE == 1 && ((seen_rdtsc == 2 && instr_flushed_beginning < EXTRA_ARR_SIZE) || seen_rdtsc == 4) && instr_flushed_beginning != UNROLLING_FACTOR) 
                ...
                { 
                    ...
                    #if(SENDER_PAYS_SOCKET_LATENCY == 1)
                        // @socket_latency_to_avoid_clflush: Adding socket latency just for sender cpu.
                        if(NUM_CPUS > 1 && cpu == 1)
                            cpu_cycle_to_trigger_clflush[cpu][num] = current_core_cycle[cpu] + cycles_to_wait_for_before_triggering_clflush_with_socket_latency;
                   #endif
                    #if(EVERYONE_PAYS_SOCKET_LATENCY == 1)
                        // @socket_latency_to_avoid_clflush: Adding socket latency just for sender and receiver both.
                        if(NUM_CPUS > 1)
                            cpu_cycle_to_trigger_clflush[cpu][num] = current_core_cycle[cpu] + cycles_to_wait_for_before_triggering_clflush_with_socket_latency;
                   #endif
                }
