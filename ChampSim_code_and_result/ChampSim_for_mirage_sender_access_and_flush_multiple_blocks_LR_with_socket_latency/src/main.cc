...
// @socket_latency_to_avoid_clflush
int clflush_check_flag = 0, dont_let_receiver_proceed = 0;
...
int cycles_to_wait_for_before_triggering_clflush = 20, cycles_to_wait_for_before_triggering_clflush_with_socket_latency = 1100;
...
int main(int argc, char** argv)
{
    ...
    if(SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD == 0)
    {   
        cycles_to_wait_for_before_triggering_clflush_with_socket_latency = 1100;
    }
    else if(SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD == 1)
    {    
        cycles_to_wait_for_before_triggering_clflush_with_socket_latency = 5600;
    }
    else
    {
        cout<<"Flag for Socket latency is not set properly."<<endl;   
        assert(0);
    }
   ...
   cout<<"SENDER_PAYS_SOCKET_LATENCY: "<<SENDER_PAYS_SOCKET_LATENCY<<" EVERYONE_PAYS_SOCKET_LATENCY: "<<EVERYONE_PAYS_SOCKET_LATENCY<<" SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD: "<<SOCKET_LATENCY_WITH_SYNCHRONIZATION_OVERHEAD<<endl;
	 cout<<"cycles_to_wait_for_before_triggering_clflush_with_socket_latency: "<<cycles_to_wait_for_before_triggering_clflush_with_socket_latency<<endl;
          ...
          {     if(NUM_CPUS > 1 && SETUP_DONE[0] == 1 && SETUP_DONE[1] == 1) //XXX Signify that receiver is done with cache prime and sender is ready to send first bit.  
               ...
               //Receiver i.e. cpu 0 has seen wait once and sender has seen wait twice. Implementation dependent.
                ...
                { 
                          ...
                          // @socket_latency_to_avoid_clflush: Check sender's clflush has completed or not
                          if(FIRST_TIME_CACHE_SETUP_DONE == 1 && TURN == 0)
                          {
                                // Checking only for sender's pending clflush.
																for(int num=0; num<MAX_CLFLUSH_POSSIBLE; num++)
																{   if(clflush_on_cpu[1][num] != 0)
																		{
                                        cout<<"Checking for clflush completion."<<endl;
                                        clflush_check_flag = 1;
																		}
																}
                          } 
...
#endif
								// @socket_latency_to_avoid_clflush: Check sender's clflush has completed or not
								if(clflush_check_flag == 1 && NUM_CPUS > 1)
								{
											int incomplete_clflush_count = 0;
											// Checking only for sender's pending clflush.
											for(int num=0; num<MAX_CLFLUSH_POSSIBLE; num++)
											{   if(clflush_on_cpu[1][num] != 0)
													{
															incomplete_clflush_count ++;
													}
											}
											if(incomplete_clflush_count == 0)
											{		clflush_check_flag = 0;
                          dont_let_receiver_proceed = 0;
                          cout << "==== Let receiver proceed ======"<<" current_core_cycle: "<<current_core_cycle[i]<<endl;
                      }
                      else
											{		
                          clflush_check_flag = 1;
                          dont_let_receiver_proceed = 1;
                      }
								}
...
#endif
                // @socket_latency_to_avoid_clflush: dont_let_receiver_proceed.
                // fetch unit
                if (ooo_cpu[i].ROB.occupancy < ooo_cpu[i].ROB.SIZE  && cpu_on_wait[i] == 0 && fence_called_on_cpu[i] == 0 && dont_let_receiver_proceed == 0) //cpu_on_wait is 1 when core is on sleep
                {
