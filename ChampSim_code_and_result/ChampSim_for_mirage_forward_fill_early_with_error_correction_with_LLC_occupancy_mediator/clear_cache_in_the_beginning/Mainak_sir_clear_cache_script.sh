arr_size_multiples_max=1.8     #$1
num_iterations_max=3         #$2
...
rm -f clear_cache_result_files/Mainak_sir_clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}.txt  #Force remove everything inside this directory.

echo "arr_size_multiples,rand_seed,num_iterations,LLC_OCCUPANCY,Total_cycles,LLC_Filling_cycles,Thrashing_cycles,Probe_cycles" >> clear_cache_result_files/Mainak_sir_clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}.txt #; done    
#exit
...
#while [[ ${num_iterations} -le ${num_iterations_max} ]]  # XXX bc should be used for floating point variables.
while [ $(bc <<< "${num_iterations} <= ${num_iterations_max}") -eq 1 ]; do

		arr_size_multiples=1.8
    ...
    while [ $(bc <<< "${arr_size_multiples} <= ${arr_size_multiples_max}") -eq 1 ]; do
				...
				./clear_cache.sh ${arr_size_multiples} ${num_iterations} ${path}
            ...
            # Build the binary only once.
                 ...
                 ./build_1core.sh ${sim} champsim.trace_clear_cache_${arr_size_multiples}.gz 0 ${rand_seed} 0 0 0 0
						...
						#for i in ${value[@]}; do 
						printf "%s" "${arr_size_multiples}," "${rand_seed}," "${num_iterations}," "${llc_occ_value[@]}," "${total_cycle[@]}," "${LLC_filling_cycle}," "${access_cycle[@]}," "${probe_cycle[@]}" >> clear_cache_result_files/Mainak_sir_clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}.txt #; done 
            echo "" >> clear_cache_result_files/Mainak_sir_clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}.txt 
    ...
    num_iterations=`bc -l<<< " ${num_iterations} + 1"` # expr command does not work on floating point numbers.
