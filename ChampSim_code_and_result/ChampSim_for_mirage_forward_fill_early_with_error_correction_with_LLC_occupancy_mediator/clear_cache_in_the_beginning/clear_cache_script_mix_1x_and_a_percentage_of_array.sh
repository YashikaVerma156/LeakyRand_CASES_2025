...
do
    ...
    while [ $(bc <<< "${arr_size_multiples} < ${arr_size_multiples_max}") -eq 1 ]; do
				...
				./clear_cache_mix_1x_and_a_percentage_of_array.sh ${arr_size_multiples} ${num_iterations_max} ${num_iterations_1} ${path}
            ...
            # Build the binary only once.
						if [ ${built_binary_once} -lt 1 ]; then
								 # build the binary
                 ./build_1core.sh ${sim} champsim.trace_clear_cache.gz 0 ${rand_seed} 0 0 0 0

								 # increment the value
								 built_binary_once=`expr ${built_binary_once} + 1`
						fi

						./run_1core.sh ${sim} champsim.trace_clear_cache.gz 0 ${rand_seed} 0 0 0 0
						
						val=`grep -nri "Calculat" result_random_1_champsim.trace_clear_cache.gz.txt | head -1 | awk '{print $8 }'`
						val_1=`grep -nri "Calculat" result_random_1_champsim.trace_clear_cache.gz.txt | tail -1 | awk '{print $8 }'`
						tot_cycle=`grep -nri "Finished CPU 0 instructions:" result_random_1_champsim.trace_clear_cache.gz.txt | awk '{print $7 }'`
						...
						LLC_filling_cycle=`grep -nr "Retiring instr_id: ${a} " result_random_1_champsim.trace_clear_cache.gz.txt | head -1 | awk '{print $10}'`

						# Instr_id of next occurrence of fence.
						thrash_fin_instr=`grep -nr "fence: 1" result_random_1_champsim.trace_clear_cache.gz.txt | head -5 | tail -1 | awk '{print $11}'`

						# Retirement cycle of fence after thrashing.
						thrashing_finish_cycle=`grep -nr "Retiring instr_id: ${thrash_fin_instr} " result_random_1_champsim.trace_clear_cache.gz.txt | head -1 | awk '{print $10}'`
						...
						#for i in ${value[@]}; do 
						printf "%s" "${arr_size_multiples}," "${rand_seed}," "${num_iterations_max}," "${num_iterations_1}," "${llc_occ_value[@]}," "${llc_occ_value_1[@]}," "${total_cycle[@]}," "${LLC_filling_cycle}," "${access_cycle[@]}," "${probe_cycle[@]}" >> clear_cache_result_files/clear_cache_stats_mix_1x_and_a_percentage_of_array_${arr_size_multiples_max}_${num_iterations_max}_${num_iterations_max_1}_${rand_seed_iterations_max}.txt #; done 
            echo "" >> clear_cache_result_files/clear_cache_stats_mix_1x_and_a_percentage_of_array_${arr_size_multiples_max}_${num_iterations_max}_${num_iterations_max_1}_${rand_seed_iterations_max}.txt 
