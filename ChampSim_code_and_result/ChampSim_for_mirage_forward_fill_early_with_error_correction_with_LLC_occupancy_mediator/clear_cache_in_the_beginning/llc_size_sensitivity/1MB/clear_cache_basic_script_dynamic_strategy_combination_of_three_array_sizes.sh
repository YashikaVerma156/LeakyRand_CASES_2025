...
if [ "$#" -ne 6 ]; then
    ...
    exit 1
        ...
        path=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_forward_fill_early_with_error_correction_with_LLC_occupancy_mediator/clear_cache_in_the_beginning/llc_size_sensitivity/1MB
        ...
        ./commands_receiver_LR_clear_cache_dynamic_strategy_three_array_sizes_diff_llc_size.sh ${arr_size_multiples} ${num_iterations_mid1} ${num_iterations_mid2} ${num_iterations} ${path} ${mid_number1} ${mid_number2} ${LLC_NUM_BLOCKS}
        sim=`grep -nr "Count_instr:" clear_cache.txt | awk '{ print $4 }'`

        cat clear_cache.txt

        echo "sim is: ${sim}"
        cd ../../../../../ChampSim_for_mirage_forward_fill_early_with_error_correction_with_LLC_occupancy_mediator/clear_cache_in_the_beginning/llc_size_sensitivity/1MB
       ...
       # XXX The following lines of code has not been put into use. Calculate the total cycles = fill_cycles + probe_cycles manually from the result file after subtracting the cycles spent in filling the LLC in the beginning. 
        ...
        echo "Cycles spent in CRFill: $crfill_cycle"  >> temp_1MB.txt
        echo "Cycles spent in CRProbe: $crprobe_cycle" >> temp_1MB.txt
