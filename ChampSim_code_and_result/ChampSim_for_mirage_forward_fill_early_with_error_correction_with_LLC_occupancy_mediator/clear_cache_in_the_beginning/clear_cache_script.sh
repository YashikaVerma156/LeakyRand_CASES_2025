arr_size_multiples_max=1.7        #$1
num_iterations_max=5             #$2
...
mkdir -p clear_cache_result_files_updated # no error if the directory already exists.
rm -f clear_cache_result_files_updated/clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}_new.txt  #Force remove everything inside this directory.

echo "arr_size_multiples,rand_seed,num_iterations,LLC_OCCUPANCY,Total_cycles,LLC_Filling_cycles,Thrashing_cycles,Probe_cycles" >> clear_cache_result_files_updated/clear_cache_stats_${arr_size_multiples_max}_${num_iterations_max}_${rand_seed_iterations_max}_new.txt #; done    
#exit
