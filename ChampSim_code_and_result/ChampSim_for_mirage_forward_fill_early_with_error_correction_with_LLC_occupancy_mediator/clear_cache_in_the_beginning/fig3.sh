...
rm latest_clear_cache_stats_${num_iterations}_new.txt
...
do
	...
	if [ ${arr_sizes} -eq 1 ]; then
            rand_seed=32088
	elif [ ${arr_sizes} -eq 2 ]; then
            rand_seed=31319
	elif [ ${arr_sizes} -eq 3 ]; then
            rand_seed=22979
	elif [ ${arr_sizes} -eq 4 ]; then
            rand_seed=27245
	elif [ ${arr_sizes} -eq 5 ]; then
            rand_seed=23186
	fi
        ...
        thrashing_finish_cycle=`grep -nr "Retiring instr_id: ${thrash_fin_instr} " result_random_1_champsim.trace_clear_cache_${arr_size_multiples}.gz.txt | head -1 | awk '{print $10}'`
         ...
         printf "%s" "${arr_size_multiples}," "${rand_seed}," "${num_iterations}," "${llc_occ_value[@]}," "${total_cycle[@]}," "${LLC_filling_cycle}," "${access_cycle[@]}," "${probe_cycle[@]}" >> latest_clear_cache_stats_${rand_seed_iterations_max}_new.txt #; done 
         echo "" >> latest_clear_cache_stats_${rand_seed_iterations_max}_new.txt 
