...
rm latest_clear_cache_stats_${num_iterations}_new_fig11a.txt

for arr_sizes in 1 1.2
do
    arr_size_multiples=${arr_sizes} 
    built_binary_once=0
        rand_seed_iterations_max=1
	if [ ${arr_sizes} == "1.2" ]; then
            num_iterations=9
	fi
	if [ ${arr_sizes} == "1" ]; then
            num_iterations=1
	...
	if [ ${arr_sizes} == "1.2" ]; then
        ...
        thrashing_finish_cycle=`grep -nr "Retiring instr_id: ${thrash_fin_instr} " result_random_1_champsim.trace_clear_cache_${arr_size_multiples}.gz.txt | head -1 | awk '{print $10}'`
         ...
         printf "%s" "${arr_size_multiples}," "${rand_seed}," "${num_iterations}," "${llc_occ_value[@]}," "${total_cycle[@]}," "${LLC_filling_cycle}," "${access_cycle[@]}," "${probe_cycle[@]}" >> latest_clear_cache_stats_${rand_seed_iterations_max}_new_fig11a.txt #; done 
         echo "" >> latest_clear_cache_stats_${rand_seed_iterations_max}_new_fig11a.txt 
