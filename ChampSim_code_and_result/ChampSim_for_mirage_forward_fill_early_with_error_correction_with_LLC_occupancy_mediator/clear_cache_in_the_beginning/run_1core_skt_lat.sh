...
do
	...
	(./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed}  -traces $mix1) > clear_cache_in_the_beginning/result_${repl}_${CORE}_${trace1}_${latency}_${skt_lat}_${skt_lat_sync}.txt
