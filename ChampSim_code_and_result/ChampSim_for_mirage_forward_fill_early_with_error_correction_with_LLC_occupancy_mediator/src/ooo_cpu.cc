...
void O3_CPU::handle_branch()
{
    ...
    while (continue_reading) {
        ...
        /*        else if(LLC_OCC_CHK_PHASE == 1 && ((seen_rdtsc == 2 && instr_flushed_beginning < EXTRA_ARR_SIZE) || seen_rdtsc == 4) && instr_flushed_beginning != UNROLLING_FACTOR) 
                ...
                {
                    if(INC_ARR_LEN > 0 /*&& bit_revealed_is == 1*/) // Considering there is possibility of error in revealing the bit value.
                    {
                        seen_llc_occ_chk ++;
                        continue_reading = 0;
                ...
                { 
                    ...
                    cpu_cycle_to_trigger_clflush[cpu][num] = current_core_cycle[cpu] + cycles_to_wait_for_before_triggering_clflush; 
                    if(CLEAR_CACHE_WITH_SKT_LAT == 1)
                        cpu_cycle_to_trigger_clflush[cpu][num] = current_core_cycle[cpu] + cycles_to_wait_for_before_triggering_clflush_with_skt_lat; 
                    else if(CLEAR_CACHE_WITH_SKT_LAT_SYN == 1)
                        cpu_cycle_to_trigger_clflush[cpu][num] = current_core_cycle[cpu] + cycles_to_wait_for_before_triggering_clflush_with_skt_lat_sync; 
                }
                ...
                if(current_instr.fence == 1)
                {
                    if(cpu == 0) 
                    {
                                ...
                                // Below condition is for dynamic_strategy. 
                                fence_is_seen ++;
                                Calculate_LLC_occupancy_dyn_algo ++;
                            }
                    }
                    ...
                    fence_instr[cpu] = instr_unique_id;
										if(DETAILED_PRINT || FA_LRU_CACHE)
                ...
                if(current_instr.rdtsc == 1)
                {
                    ...
                    if(FIRST_TIME_CACHE_SETUP_DONE)
                    {
                       // The below condition is used in recording the error stats.
                        ...
                        {
                            if( ( ( (current_core_cycle[cpu] - prev_rdtsc_counter) > HIT_MISS_THRESHOLD) //|| 
  //Assuming HIT_MISS_THRESHOLD for UNROLLING_FACTOR == 1 and HIT_MISS_THRESHOLD_LLC_OCC_CHK are the same.
                              ...
                              (LLC_OCC_CHK_PHASE == 1 && (current_core_cycle[cpu] - prev_rdtsc_counter) > HIT_MISS_THRESHOLD_LLC_OCC_CHK))
                            {   
                                if(bit_revealed_is == 1)
                ...
                arch_instr.instr_id = instr_unique_id;
								 if(DETAILED_PRINT || CLEAR_CACHE /*|| LLC_OCC_CHK_PHASE || CHANNEL_BW_ERR_PRINT*/)
...
void O3_CPU::retire_rob()
{
    ...
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " is retired" << endl; });
				...
				if(DETAILED_PRINT || CLEAR_CACHE || (ROB.entry[ROB.head].instr_id >= range_start && ROB.entry[ROB.head].instr_id <= range_end))
				...
				if(fence_instr[cpu] == ROB.entry[ROB.head].instr_id && cpu == 0 && fence_instr[cpu] > 1)
						cout<<"FFFFFFF cpu "<<cpu<<" Retiring fence, current core cycle: "<<current_core_cycle[cpu]<<" instr_id: "<<fence_instr[cpu]<<endl;
