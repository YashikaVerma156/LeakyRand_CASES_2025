...
# b) MAKE SURE that in this script the seeds_t*.txt file is set as per the requirement.                        #
################################################################################################################
...
if [ "$#" -ne 7 ]; then
    echo "Usage: $0 <LLC_size> <bench_test> <build_2_core_binary> <generate_receiver_trace> <generate_sender_trace> <str_num> <num_prl>"
    echo "LLC_size should be either '1' for 1MB or '4' for 4MB."
    ...
    echo "str_num controls how many experiments per configuration are run. If bench_test is 1, 1 <= str_num <= 500, If bench_test is 0, 1 <= str_num <= 250"
    echo "num_prl tells how many parallel experiments to run."
    exit 1
fi

# Check if the LLC_size is either 1 or 4
if [ "$1" -ne 1 ] && [ "$1" -ne 4 ]; then
    echo "The value of LLC_size must be either 1 for 1MB or 4 for 4MB."
    exit 1
fi
...
if [ "$LLC_size" -eq 1 ];
then 
    sender_arr_size_list=(4915) # We are testing for sender size: 30% LLC size only.
    llc_sets=1024
elif [ "$LLC_size" -eq 4 ];
then
    sender_arr_size_list=(19660) # We are testing for sender size: 30% LLC size only.
    llc_sets=4096
fi
...
then
    ...
    do
        ...
        ./generate_sender_trace_LLC_size_sensitivity.sh ${size} $START_POINT $STOP_POINT $bench_test $llc_sets $LLC_size 224576
    done 
    #exit
fi
