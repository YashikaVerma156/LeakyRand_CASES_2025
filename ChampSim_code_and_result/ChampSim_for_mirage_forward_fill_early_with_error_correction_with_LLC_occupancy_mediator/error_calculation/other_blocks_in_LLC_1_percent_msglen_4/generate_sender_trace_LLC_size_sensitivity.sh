...
do
    echo "string_num: $i"
		...
		./commands_sender_all_1_seed_with_benchmark_suit_LLC_size_sensitivity.sh $i ${string_size} ${sender_arr_size} ${repetition_coding} ${differential_signalling} ${path_to_copy_trace} ${bench_test}
    ...
    if [ $bench_test == 0 ]
    then
        trace_sender=champsim.trace_sender_${string_size}_${i}_${sim_count}_arr_size_${sender_arr_size}_seed_train.gz
    elif [ $bench_test == 1 ]
    then
        trace_sender=champsim.trace_sender_${string_size}_${i}_${sim_count}_arr_size_${sender_arr_size}_seed_test.gz
    fi

    if [ $test_the_trace -eq 1 ];
    then
        ./run_build_1core_LLC_size_sensitivity.sh ${sim_count} ${trace_sender} ${seed} ${string_size} ${repetition_coding} ${differential_signalling} ${llc_sets} ${path_to_copy_trace} 
    fi    
