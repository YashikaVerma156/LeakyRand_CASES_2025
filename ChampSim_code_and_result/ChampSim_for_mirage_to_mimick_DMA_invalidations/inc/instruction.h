...
class input_instr {
  public:
    ...
    // extra flags for covert channel.
    int warmup;
    int wait;
    int fence;
    int rdtsc;
    ...
    int double_probe_start;
    int double_probe_end;
    int repeat_hole_fill;
    int ipc_count_start;
...
class ooo_model_instr {
  public:
    ...
    uint8_t is_branch,
		...
		int warmup, wait, fence, rdtsc;
    uint64_t clflush;
    int double_probe_start, double_probe_end, error_correction, llc_occ_chk, llc_occ_chk_end, repeat_hole_fill, ipc_count_start;
