...
int CURR_ERR_CORR_ITR = 0, MISSES_OBSERVED = 0, ERROR_CORRECTION_PHASE_LAST = 0, BIT_REVEALED_LAST = 0, CURR_LLC_OCC_CHK_ITR =0, ITR_TO_FILL_HOLE=0;
...
//@NOISE_CODE
uint64_t NOISE_BLOCK_START_ADDRESS = 987230208/* hard-coded */, noise_trigger_address = NOISE_BLOCK_START_ADDRESS, bit_count = 0;
...
//@NOISE_CODE
void allocate_physical_pages_to_noise_addresses()
{
    // This range can be fixed as we already know about the virtual address range and array address range of receiver for this configuration.
    // Open file and allocate physical pages to all the virtual addresses of noise blocks. It should not hit with any already present entry in the page table.
    uint64_t noise_address = NOISE_BLOCK_START_ADDRESS;
    //va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
    for(int i=0; i<512; i++)
    {
        va_to_pa(0/*Making it a receiver code part.*/, 0, noise_address, noise_address >> LOG2_PAGE_SIZE);
        //cout <<"pa allocated to noise address is: "<< pa <<" allocated_pages: "<<allocated_pages<< endl;
        noise_address = noise_address + 64;
    }
}
//@NOISE_CODE
// Function to trigger noise.
void trigger_noise()
{
    // Open the file.
    // Translate it.
    uint64_t pa = va_to_pa(0/*Making it a receiver code part.*/, 0, noise_trigger_address, noise_trigger_address >> LOG2_PAGE_SIZE);
    //cout <<"pa retrieved to noise address is: "<< pa <<" allocated_pages: "<<allocated_pages<< endl;
    // Add the packet generated at the head of LLR RQ. Take care of the necessary fields of the packet, like fill level. Do check for the LLC RQ occupancy, it should be zero.
    // The packet should pass through the hierarchy normally.
    cout <<"LLC RQ occupancy: "<<uncore.LLC.RQ.occupancy << endl; 
    PACKET data_packet;
    data_packet.fill_level = FILL_LLC;
    data_packet.cpu = 0;
    //data_packet.data_index = LQ.entry[lq_index].data_index;
    //data_packet.lq_index = lq_index;
    data_packet.address = pa >> LOG2_BLOCK_SIZE;
    data_packet.full_addr = pa;
    data_packet.instr_id = 0;
    ...
    data_packet.event_cycle = current_core_cycle[0];

    //PACKET_QUEUE *queue;
    uncore.LLC.add_rq(&data_packet);
   
    //cout <<"LLC RQ occupancy: "<<uncore.LLC.RQ.occupancy << endl;
    //Increment the unique block address of the noise. There is no reuse in the noise addresses. 
    noise_trigger_address += 64;
    cout << "Triggered noise" << endl;
}
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
{
    ...
    {
        if(va == 987230208)
            cout << "No stall cycles added. "<<endl;
        else
    ...
    cout << "cpu: " << cpu << " allocated unique_vpage: " << hex << unique_vpage << " to ppage: " << ppage << dec << endl;
	  cout << hex << "LOG2_PAGE_SIZE: "<<LOG2_PAGE_SIZE <<" ppage: "<< ppage << " pa: " << pa<<" va: " << va << dec <<endl;
...
// TODO: can we initialize these variables from the class constructor?
    ...
    ooo_cpu[0].REDUCED_ARR_SIZE = (ARR_MAX_LIMIT - (EXTRA_ARR_SIZE * 64 ));
    cout << "ARR_MAX_LIMIT: "<<ARR_MAX_LIMIT<<" EXTRA_ARR_SIZE: "<<EXTRA_ARR_SIZE<<" ooo_cpu[0].REDUCED_ARR_SIZE: "<<ooo_cpu[0].REDUCED_ARR_SIZE<<endl;
                ...
                if(NUM_CPUS > 1 && SETUP_DONE[0] == 1 && SETUP_DONE[1] == 2) //XXX Signify that receiver is done with cache prime and sender is done with sending the first bit.	
                    {
                        if(FIRST_TIME_CACHE_SETUP_DONE == 0)
											  {	
                            allocate_physical_pages_to_noise_addresses();
                              ...
                              if(i == 1)
                              {
                                   ...
                                   {   
                                       cout <<" cycle: "<<current_core_cycle[i]<< " bit received is: "<< bit_revealed_is << endl;
																			 //@NOISE_CODE
                                       bit_count ++;
                                       if(bit_count % NOISE_TRIGGER_POINT == 0 && NOISE_TRIGGER_POINT != 0)
                                           trigger_noise(); 
                                   }
                                   receiver_checks_for_a_miss = 0; 
                                   bit_revealed_is=0;
