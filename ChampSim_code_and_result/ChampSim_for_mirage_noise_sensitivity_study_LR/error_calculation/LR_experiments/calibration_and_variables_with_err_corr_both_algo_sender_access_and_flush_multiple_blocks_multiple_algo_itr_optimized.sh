...
if [ $other_blocks -eq 326 ]; then
	...
	if [ $unroll_fact -eq 16 ]; then
			...
			if [ ${algo_trigger_point} -eq 128 ]; then
					...
					arr_size=259264
			elif [ ${algo_trigger_point} -eq 514 ]; then
					arr_size=259272
			fi
...
mkdir -p /home/yashikav/Desktop/Mirage_project/fig7/champsim_results
mkdir -p /home/yashikav/Desktop/Mirage_project/fig7/champsim_results/LR_ERR_CORR_2core_results_both_algo_${err_corr_iterations}_access
...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_for_err_corr_both_algo_2core_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized_update_arr_size.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} ${noise_trigger_point}
...
./trigger_LR_WITH_ERR_CORR_both_algo_with_benchmark_suit_sender_access_and_flush_multiple_blocks_multiple_algo_itr_optimized.sh ${unroll_fact} ${START_POINT} ${END_POINT} ${num_prl} ${benchmark_test} ${string_size} ${other_blocks} ${arr_size} ${algo_trigger_point} ${hit_miss_threshold} ${err_corr_iterations} ${noise_trigger_point}
