...
do
	...
	(./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed} -traces $mix1 $mix2) > ${Result_dir}/result_${repl}_${CORE}_${trace1}_${trace2}_noise_point_${noise_trigger_point}.txt
