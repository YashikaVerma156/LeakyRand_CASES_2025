...
if [ "$#" -ne 3 ]; then
    echo "Usage: $0 <benchmark_test> <other_block> <str_num>"
    echo "benchmark_test should be either 1 or 0, 1: if sender traces for test_benchmark_set is to be generated, 0: if sender traces for train_benchmark_set is to be generated. other_block should be either 31 or 326, 31 : 0.1% LLC unoccupancy. 326 : 1% LLC unoccupancy. str_num decides the number of traces to be generated."
    exit 1
fi
...
if [ "$2" -ne 326 ]; then
    echo "The value of other_block must be 326."
    exit 1
fi
...
if ! [[ $2 =~ $integer_regex ]]; then
    echo "The value of other_block must be an integer."
    exit 1
fi

# Check if the argument matches the integer pattern
if ! [[ $3 =~ $integer_regex ]]; then
    echo "The value of str_num must be an integer."
    exit 1
fi
...
err_corr_interval=514
for err_corr_iterations in 100 800 #8 16 32 64 128 256 512 #8 16 32 64 128
do
				if [ ${benchmark_test} -eq 1 ];
				then
						i=${str_num}
				else
						i=${str_num}  #250  #250    #250
				fi
