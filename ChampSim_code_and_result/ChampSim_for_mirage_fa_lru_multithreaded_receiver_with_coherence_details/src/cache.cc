	...
	{     
        ...
        //if(MSHR.entry[index].instr_id == 7458) cout <<"stall_cycle of cpu: "<<stall_cycle[MSHR.entry[index].cpu]<< " returned from cache_type: " << (int)cache_type<<" core cycle: "<<current_core_cycle[cpu]<<endl;
            ...
            if (MSHR.entry[index].fill_level < fill_level) {
                ...
                if(cache_type == IS_LLC && NUM_CPUS > 1)
                {
                    for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
                    {
												if(MSHR.entry[index].merged_cpus[merged_cross_cpu] == 1)
												{
                            cout << "returned from merge in line 343" <<endl;
														if (MSHR.entry[index].instruction)
																upper_level_icache[merged_cross_cpu]->return_data(&MSHR.entry[index]);
														else // data
																upper_level_dcache[merged_cross_cpu]->return_data(&MSHR.entry[index]);
												    MSHR.entry[index].merged_cpus[merged_cross_cpu] = NUM_CPUS;
                        }  
                    }
                }
								returned_data_count ++;
        ...
        if (way >= 0 && way < NUM_WAY) { // writeback hit (or RFO hit for L1D)
            ...
            if (WQ.entry[index].fill_level < fill_level) {   // @Question, will this ever be the case? Is this unnecessary. fill_level will never be less for WQ pkt.
                ...
                if(cache_type == IS_LLC && NUM_CPUS > 1)
                {
                    for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
                    {
												if(WQ.entry[index].merged_cpus[merged_cross_cpu] == 1)
												{
                            cout << "returned from merge in line 810" <<endl;
														if (WQ.entry[index].instruction) 
																upper_level_icache[merged_cross_cpu]->return_data(&WQ.entry[index]);
														else // data
																upper_level_dcache[merged_cross_cpu]->return_data(&WQ.entry[index]);
												    WQ.entry[index].merged_cpus[merged_cross_cpu] = NUM_CPUS;
                        }  
                    }
                }
            }
										...
										if (do_fill) {
												...
												if(cache_type == IS_LLC && NUM_CPUS > 1)
												{
														for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
														{
																if(WQ.entry[index].merged_cpus[merged_cross_cpu] == 1)
																{
																		cout << "returned from merge in line 1131" <<endl;
																		if (WQ.entry[index].instruction)
																				upper_level_icache[merged_cross_cpu]->return_data(&WQ.entry[index]);
																		else // data
																				upper_level_dcache[merged_cross_cpu]->return_data(&WQ.entry[index]);
...
void CACHE::handle_read()
{
        ...
        //cout << "cache_type: "<< (int) cache_type << " MAX_READ: "<<MAX_READ <<endl;
                    ...
                    //cout <<"i: "<<i<<" entry address: "<<RQ.entry[index].address<<" instr_id: "<<RQ.entry[index].instr_id<<endl;
                      ...
                      cout << "transferred to L1D processed queue: " << RQ.entry[index].ip<<" current_core_cycle[0]: "<<current_core_cycle[0] << endl;
										...
										if(cache_type == IS_LLC && NUM_CPUS > 1)
										{
												for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
												{
														if(RQ.entry[index].merged_cpus[merged_cross_cpu] == 1)
														{
																cout << "returned from merge in line 1291" <<endl;
																if (RQ.entry[index].instruction)
																		upper_level_icache[merged_cross_cpu]->return_data(&RQ.entry[index]);
																else // data
																		upper_level_dcache[merged_cross_cpu]->return_data(&RQ.entry[index]);
																RQ.entry[index].merged_cpus[merged_cross_cpu] = NUM_CPUS;
														}
												}
										}
				...
				MSHR.entry[mshr_index].lq_index_depend_on_me.join (RQ.entry[index].lq_index_depend_on_me, LQ_SIZE);
                            ...
                            // Taking care of requests of different cpus merging at LLC.
                            if(cache_type == IS_LLC && RQ.entry[index].cpu != MSHR.entry[mshr_index].cpu && (MSHR.entry[mshr_index].store_merged == 1 || MSHR.entry[mshr_index].load_merged == 1))
                            {
                                MSHR.entry[mshr_index].merged_cpus[RQ.entry[index].cpu] = 1;
                            }
                        }
                        else {
                            if (RQ.entry[index].instruction) {
				    ...
				    MSHR.entry[mshr_index].rob_index_depend_on_me.join (RQ.entry[index].rob_index_depend_on_me, ROB_SIZE);
                                ...
                                // Taking care of requests of different cpus merging at LLC.
																if(cache_type == IS_LLC && RQ.entry[index].cpu != MSHR.entry[mshr_index].cpu)
																{
																		MSHR.entry[mshr_index].merged_cpus[RQ.entry[index].cpu] = 1;
																}
                            }
                    ...
                    if (PQ.entry[index].instruction) 
                        ...
                        upper_level_dcache[prefetch_cpu]->return_data(&PQ.entry[index]);
								...
								if(cache_type == IS_LLC && NUM_CPUS > 1)
								{
										for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
										{
												if(PQ.entry[index].merged_cpus[merged_cross_cpu] == 1)
												{
                            cout << "returned from merge in line 1569" <<endl;
														if (PQ.entry[index].instruction)
																upper_level_icache[merged_cross_cpu]->return_data(&PQ.entry[index]);
														else // data
																upper_level_dcache[merged_cross_cpu]->return_data(&PQ.entry[index]);
														PQ.entry[index].merged_cpus[merged_cross_cpu] = NUM_CPUS;
												}
										}
								}
...
//		if(cache_type == IS_LLC)
    ...
    if (packet->instruction) 
		...
		if(DETAILED_PRINT || (packet->instr_id >= range_start && packet->instr_id <= range_end) || packet->cpu == 2)
...
{		
    ...
    if(DETAILED_PRINT || (packet->instr_id >= range_start && packet->instr_id <= range_end))
    {
       if(match_way == UINT32_MAX)
           ...
           cout << "it is a hit ";
       cout<<"Inside check_hit, cache: "<<(int)cache_type<<", for cpu: "<< packet->cpu<<" addr: "<<packet->address<<" full_addr: "<<packet->full_addr<<" set: "<<set<<" way: "<<match_way<<" current_core_cycle[0]: "<<current_core_cycle[0]<<"  packet->instr_id: "<<packet->instr_id<<" packet->ip: "<<packet->ip<<endl;
    }

   // @helper core: the helper core should see only misses in LLC for array.
   if(cache_type == IS_LLC && packet->ip == 0x4017b5 && packet->cpu == 2 && !(packet->instruction))
   {    if(match_way != UINT32_MAX) 
           assert(0);
   }

   // @helper core: the receiver core should not get hit in private caches for array while probing.
   if((cache_type == IS_L1D || cache_type == IS_L2C) && packet->ip == 0x4017d0 && packet->cpu == 0 && !(packet->instruction))
   {    if(match_way != UINT32_MAX) 
           assert(0);
   }
    ...
    if(cache_type == IS_LLC && /*FIRST_TIME_CACHE_SETUP_DONE &&*/ match_way == UINT32_MAX && (CHANNEL_BW_ERR_PRINT || ((LR_ERR_CORR_BOTH_ALGO || LR_NO_ERR_CORR) && CACHE_FILL_DONE)))
    {
        ...
        cout <<"found a miss for CPU: "<< packet->cpu <<" packet->address: "<< hex<<packet->address<<" full_addr: "<<packet->full_addr<<dec<< " cycle: " << current_core_cycle[packet->cpu]<<" receiver_checks_for_a_miss "<<receiver_checks_for_a_miss<<" instr_id: "<<packet->instr_id <<endl; 
    }
    ...
    if (index != -1) {
        ...
        if (packet->instruction) {
            ...
            RQ.entry[index].instr_merged = 1;
						...
						// Taking care of requests of different cpus merging at LLC.
						if(cache_type == IS_LLC && RQ.entry[index].cpu != packet->cpu)
						{
								RQ.entry[index].merged_cpus[packet->cpu] = 1;
						}
        ...
        if (packet->fill_level < fill_level) {
            ...
            if (packet->instruction)
				...
				if(cache_type == IS_LLC && NUM_CPUS > 1)
				{
						for(int merged_cross_cpu = 0; merged_cross_cpu < NUM_CPUS; merged_cross_cpu ++)
						{
								if(packet->merged_cpus[merged_cross_cpu] == 1)
								{
										cout << "returned from merge in line 2400" <<endl;
										if (packet->instruction)
												upper_level_icache[merged_cross_cpu]->return_data(&RQ.entry[index]);
										else // data
												upper_level_dcache[merged_cross_cpu]->return_data(&RQ.entry[index]);
										packet->merged_cpus[merged_cross_cpu] = NUM_CPUS;
								}
						}
				}
                    // update processed packets
