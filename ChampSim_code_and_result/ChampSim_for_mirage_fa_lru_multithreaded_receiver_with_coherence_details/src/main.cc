...
int CURR_ERR_CORR_ITR = 0, MISSES_OBSERVED = 0, ERROR_CORRECTION_PHASE_LAST = 0, BIT_REVEALED_LAST = 0, CURR_LLC_OCC_CHK_ITR = 0, ITR_TO_FILL_HOLE = 0, bit_number_received = -1, emptied_caches = 0;
...
uint64_t receiver_end_array_address_fa_lru = 0x6c2780 , receiver_start_array_address_fa_lru = 0x4c22c0;
...
uint64_t FIRST_FENCE = 0, range_start = 488 /*-1 488 */ /*1243133*/ /*296440  304127*/, range_end = 488 /*-1 488 */ /*-1*/ /*1243133*/ /*296440  304129*/;
...
// used for FA_LRU setup.
void invalidate_receiver_array_blocks()
{
    int count_l1d = 0, count_l2c = 0;
    for(int l2cset=0; l2cset< L2C_SET; l2cset++)
		{
				for(int l2cway=0; l2cway< L2C_WAY; l2cway++)
				{
					if(ooo_cpu[0].L2C.block[l2cset][l2cway].ip == 4200352 || ooo_cpu[0].L2C.block[l2cset][l2cway].ip == 4200389)
          {
					    count_l2c ++ ;
              ooo_cpu[0].L2C.block[l2cset][l2cway].valid = 0;
					}
				}
		}
    for(int l1dset=0; l1dset< L1D_SET; l1dset++)
		{
				...
				{
					if(ooo_cpu[0].L1D.block[l1dset][l1dway].ip == 4200352 || ooo_cpu[0].L1D.block[l1dset][l1dway].ip == 4200389)
          {
					    count_l1d ++ ;
              ooo_cpu[0].L1D.block[l1dset][l1dway].valid = 0;
					}
				}
		}
    cout<<"count_l1d: "<<count_l1d<<" count_l2c: "<<count_l2c<<endl;
    emptied_caches = 1;
}
...
// log base 2 function from efectiu, gives floor of log(n).
int lg2(int n)
{
    int i, m = n, c = -1;
    for (i=0; m; i++) {
...
// log base 2 function, gives ceil of log(n). 
int lg2_I(int n)
{
    int i, m = n, c = 0;
    for (i=0; m; i++) {
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
{
    ...
    uint8_t  swap = 0;
    uint64_t high_bit_mask = rotr64(cpu, lg2_I(NUM_CPUS)),
    ...
    // To facilitate sharing between two receiver threads.
    if( (va <= receiver_end_array_address_fa_lru || va >= receiver_start_array_address_fa_lru) && (cpu == 0 || cpu == 2) )
    {
        vpage = unique_vpage ;
    }
    ...
    if (pr == page_table.end()) { // no VA => PA translation found 
        ...
        //printf("Already found a translation for cpu: %d instr_id: %lx vpage: %lx  random_ppage: %lx\n",cpu, instr_id, vpage, pr->second);
        cout<<"Already found a translation for cpu: "<<cpu<<" instr_id: "<<instr_id<<" vpage: "<<vpage<<" random_ppage: "<< pr->second<<dec<<" stall_cycle[cpu]: "<<stall_cycle[cpu]<<endl;
    }
    ...
    cout<<"cpu: "<<cpu<<" allocated unique_vpage: "<<hex<<unique_vpage<<" vpage: "<<vpage<<" to ppage: "<<ppage<<dec<<" stall_cycle[cpu]: "<<stall_cycle[cpu]<<endl;
	  cout << hex << "LOG2_PAGE_SIZE: "<<LOG2_PAGE_SIZE <<" ppage: "<< ppage << " pa: " << pa<<" va: " << va << dec<<" instr_id: "<<instr_id<<endl;
...
int main(int argc, char** argv)
{
    ...
    cout<<"LOG2_DRAM_CHANNELS: "<<LOG2_DRAM_CHANNELS<<" DRAM column: "<<DRAM_COLUMNS<< "DRAM RANKS: "<<DRAM_RANKS<< " DRAM BANKS: "<<DRAM_BANKS<< " DRAM_ROWS: "<<DRAM_ROWS<< " DRAM_ROW_SIZE: "<<DRAM_ROW_SIZE<< endl;
...
// =========== Calculate LLC occupancy for initial set-up.
		...
		bit_revealed_is=0; 
            ...
            /*if(current_core_cycle[i] % 1000 == 0)
            {
            for(uint32_t ix=0; ix < ooo_cpu[i].L1D.NUM_SET; ix++)
            {
               for(uint32_t jx=0; jx < ooo_cpu[i].L1D.NUM_WAY; jx++)
               {
                   cout <<"L1D: ix: "<<ix<<" jx: "<<jx<<" "<<"block address: "<<hex<< ooo_cpu[i].L1D.block[ix][jx].address<<" tag: "<<ooo_cpu[i].L1D.block[ix][jx].tag <<" full_addr: "<<ooo_cpu[0].L1D.block[ix][jx].full_addr<<dec<<"  valid: " <<(int)ooo_cpu[0].L1D.block[ix][jx].valid<<"  dirty: "<< (int)ooo_cpu[0].L1D.block[ix][jx].dirty<<"  instr_id: "<< ooo_cpu[0].L1D.block[ix][jx].instr_id<< endl;
               } 
            }
           }*/
#if !(WAIT_IMPLEMENTATION)
          if (LR_NO_ERR_CORR || LR_ERR_CORR_BOTH_ALGO) //For LeakyRand with no error correction.
          {     if(NUM_CPUS > 1 && SETUP_DONE[0] == 1 && SETUP_DONE[1] == 1 && SETUP_DONE[2] == 1)//XXX Signify that receiver is done with cache prime and sender is ready to send first bit.
               {
               ...
               //Receiver i.e. cpu 0 has seen wait once and sender has seen wait twice and cpu 2 has seen wait once. Implementation dependent.
                if(NUM_CPUS > 1 && SETUP_DONE[0] == 1 && SETUP_DONE[1] == 2 && SETUP_DONE[2] == 2) //XXX Signify that receiver is done with cache prime and sender is done with sending the first bit.	
                {
                        if(FIRST_TIME_CACHE_SETUP_DONE == 0)
														cout<<"=========setup is done========"<<endl;
                        FIRST_TIME_CACHE_SETUP_DONE = 1;
                        /*if(emptied_caches == 0)
                            invalidate_receiver_array_blocks();*/
                }

                if(NUM_CPUS > 1 && ((i==0 && TURN==0 && cpu_on_wait[i] == 1 && cpu_on_wait[1] == 1 && cpu_on_wait[2] == 1 && SETUP_DONE[2] == 2) || (i==1 && TURN==1 && cpu_on_wait[i] == 1 && cpu_on_wait[0] == 1 && cpu_on_wait[2] == 1 && SETUP_DONE[2] == 2)))
                { 
