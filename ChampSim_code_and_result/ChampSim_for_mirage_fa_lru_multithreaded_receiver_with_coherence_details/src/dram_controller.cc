...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
    ...
    if (oldest_index != -1) { // scheduler might not find anything if all requests are already scheduled or all banks are busy
        ...
        if (row_buffer_hit)  
            {  
                 //if(current_core_cycle[queue->entry[oldest_index].cpu] > FIRST_TIME_CACHE_SETUP_CYCLES)
                 if(CHANNEL_BW_ERR_PRINT || ( (LR_NO_ERR_CORR || LR_ERR_CORR_BOTH_ALGO) && CACHE_FILL_DONE ))
                     cout<<"it is a row buffer hit ";
        ...
        //if(current_core_cycle[queue->entry[op_cpu].cpu] > 940000 /*FIRST_TIME_CACHE_SETUP_CYCLES*/)
        //    cout <<" open cpu: "<<op_cpu<<" open row is: "<<read_row<<" read_rank: "<<read_rank<<" read_bank: "<<read_bank<<" read_channel: "<<read_channel<<" instr_id: "<< queue->entry[oldest_index].instr_id<<" full_addr: "<< queue->entry[oldest_index].full_addr<< endl;

        if(queue->entry[oldest_index].instr_id == 281146 || queue->entry[oldest_index].instr_id == 325571)
        cout << "core cycle: " << current_core_cycle[op_cpu] << " new event cycle: "<<queue->entry[oldest_index].event_cycle<<" LATENCY: "<<LATENCY<<" tRP: "<< tRP<< " tRCD: "<<tRCD<< " tCAS: "<<tCAS<<endl;
...
int MEMORY_CONTROLLER::check_dram_queue(PACKET_QUEUE *queue, PACKET *packet)
{
    ...
    for (uint32_t index=0; index<queue->SIZE; index++) {
        if (queue->entry[index].address == packet->address) {
            ...
            cout << "Found a hit in DRAM queue [" << queue->NAME<<"] for instr_id: "<< packet->instr_id<<endl;
