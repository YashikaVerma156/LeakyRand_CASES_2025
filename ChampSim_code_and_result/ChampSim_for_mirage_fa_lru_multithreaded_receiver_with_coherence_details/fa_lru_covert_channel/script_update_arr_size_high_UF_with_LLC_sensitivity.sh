...
pin_output_file_receiver_helper=1_multiple_access_${receiver_num_access_in_rdtsc}_for_all_1_helper.txt
trace_receiver_prime_probe=champsim.trace_receiver_fa_lru_prime_probe.gz
trace_receiver_helper=champsim.trace_receiver_fa_lru_helper.gz
...
cd ${pin_files_path} 
./commands_receiver_fa_lru_multithreaded_with_helper.sh ${receiver_num_access_in_rdtsc} ${receiver_arr_size} ${string_size} ${path_to_copy_trace}
...
cd ${pin_files_path} 
sim_count_receiver=`grep -oE 'instrCount: [0-9]+' ${pin_output_file_receiver_helper} | awk '{print $2}'`
echo "Number of instructions traced for receiver: ${sim_count_receiver}"
cd ${back_to_exp_dir}
