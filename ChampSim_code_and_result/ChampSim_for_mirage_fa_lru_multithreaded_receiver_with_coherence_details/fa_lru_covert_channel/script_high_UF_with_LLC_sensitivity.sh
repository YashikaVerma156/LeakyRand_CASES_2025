...
hit_miss_threshold_for_err_corr=$9
...
if [ "$#" -ne 10 ]; then
   ...
   echo "benchmark_test: $benchmark_test, receiver_arr_size: $receiver_arr_size, unroll_fact: $unroll_fact, do_err_corr_at: $do_err_corr_at, other_blocks: $other_blocks, string_num: $string_num, hit_miss_threshold: $hit_miss_threshold, hit_miss_threshold_for_err_corr: ${hit_miss_threshold_for_err_corr} err_corr_iterations: $err_corr_iterations"
...
output=$(grep "#define LLC_SET 1\*2048" ../../inc/cache.h)
...
output=$(grep "#define data_LLC_WAY 16\*2048\*1" ../../inc/cache.h)
...
sed -i.bak 's/\<HIT_MISS_THRESHOLD_LLC_OCC_CHK 0\>/HIT_MISS_THRESHOLD_LLC_OCC_CHK '${hit_miss_threshold_for_err_corr}'/g' ../../inc/champsim.h
...
path_to_copy_trace=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_for_high_UF/error_calculation/LR_experiments/pin_traces
back_to_exp_dir=../../../../../ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_for_high_UF/error_calculation/LR_experiments
Result_dir=/home/yashikav/Desktop/Mirage_project/ChampSim_for_mirage_sender_access_and_flush_multiple_blocks_for_high_UF/error_calculation/LR_experiments/results
...
then
    ...
    sed -i.bak 's/\<HIT_MISS_THRESHOLD_LLC_OCC_CHK '${hit_miss_threshold_for_err_corr}'\>/HIT_MISS_THRESHOLD_LLC_OCC_CHK 0/g' ../../inc/champsim.h
...
do
   cd ${pin_files_path} 
   ./commands_receiver_LR_ERR_CORR_both_algo_with_benchmark_suit_multiple_algo_itr_optimized_for_high_UF.sh ${receiver_num_access_in_rdtsc} ${receiver_arr_size} ${extra} ${other_blocks} ${string_size} ${do_err_corr_at} ${do_llc_occ_chk_at} ${err_corr_iterations}  ${extra_for_llc_occ_chk} ${path_to_copy_trace}
  ...
  if [ $unroll_fact == 16 ] && [ $gap_number == 1456 ];
  then
          echo "gap_number: $gap_number"
  elif [ $unroll_fact == 32 ] && [ $gap_number == 1472 ];
  then
       echo "gap_number: $gap_number"
  elif [ $unroll_fact == 64 ] && [ $gap_number == 1472 ];
  then
       echo "gap_number: $gap_number"
  elif [ $unroll_fact == 128 ] && [ $gap_number == 1472 ];
  then
       echo "gap_number: $gap_number"
  elif [ $unroll_fact == 256 ] && [ $gap_number == 1424 ];
  then
       echo "gap_number: $gap_number"
  else
          sed -i.bak 's/\<MAX_ARR_INC '${max_inc_in_array}'\>/MAX_ARR_INC 0/g' ../../inc/config.h
