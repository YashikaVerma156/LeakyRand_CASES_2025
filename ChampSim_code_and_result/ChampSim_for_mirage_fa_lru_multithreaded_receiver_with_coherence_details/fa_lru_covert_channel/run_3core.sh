...
do
	repl=${repl_pol[$k]}
#  sed -i.bak 's/\<LR_NO_ERR_CORR 0\>/LR_NO_ERR_CORR 1/g' inc/champsim.h
#  ./build_champsim.sh bimodal no no ${repl} $CORE
#  sed -i.bak 's/\<LR_NO_ERR_CORR 1\>/LR_NO_ERR_CORR 0/g' inc/champsim.h
...
do
	...
	do
		...
		mix3="$mix3 ${Trace_dir}/${trace3}"
	done

	echo " (./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed} -traces $trace1 $trace2 $trace3 )"
	(./bin/bimodal-no-no-${repl}-${CORE}core -warmup_instructions $warcount -simulation_instructions $simcount ${option} -random_seed ${seed} -traces $mix1 $mix2 $mix3) > ${Result_dir}/result_${repl}_${CORE}_${trace1}_${trace2}_${trace3}.txt
