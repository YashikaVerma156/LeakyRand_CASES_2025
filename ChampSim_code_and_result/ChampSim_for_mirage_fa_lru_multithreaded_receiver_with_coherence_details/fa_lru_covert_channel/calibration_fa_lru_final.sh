...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_update_arr_size_high_UF_with_LLC_sensitivity_final.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} 
...
./trigger_fa_lru_exp_final.sh ${unroll_fact} ${START_POINT} ${END_POINT} ${benchmark_test} ${string_size} ${arr_size} ${algo_trigger_point} ${hit_miss_threshold} ${err_corr_iterations}
