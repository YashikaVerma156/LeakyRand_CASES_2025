...
#arr_size=262944  #262016 #   261904 
arr_size=262240  #262016 #   261904 
...
if [ "$output" == "arr_size is found" ]; then
    ...
    echo "The output is 'not found'"
		...
		./script_update_arr_size_high_UF_with_LLC_sensitivity_final_version_II.sh ${benchmark_test} ${arr_size} ${unroll_fact} ${algo_trigger_point} ${other_blocks} ${END_POINT} ${hit_miss_threshold} ${err_corr_iterations} 
...
# Before commenting this exit, take care of the below script.
#exit
...
./trigger_fa_lru_exp_final_version_II.sh ${unroll_fact} ${START_POINT} ${END_POINT} ${benchmark_test} ${string_size} ${arr_size} ${algo_trigger_point} ${hit_miss_threshold} ${err_corr_iterations}
