...
class PACKET {
  public:
    ...
    uint8_t  is_producer, 
             ...
             asid[2],
             type,
						 forwarded_to_next_cache_level, //Seperating forwarding(don't pay access latency) of data to next cache level and filling in cache(pay access latency).
             to_be_invalidated;
    ...
    uint32_t merged_cpus[NUM_CPUS]; // To handle merging of requests from different cores at LLC.
    ...
    PACKET() {
        ...
        returned = 0;
				forwarded_to_next_cache_level = 0;
        to_be_invalidated = 0;
        asid[0] = UINT8_MAX;
        asid[1] = UINT8_MAX;
        type = 0;
        ...
        // To handle merging of requests from different cores at LLC.
        for (uint32_t i=0; i<NUM_CPUS; i++) 
        {
            merged_cpus[i] = NUM_CPUS ;
        }
#if 0
